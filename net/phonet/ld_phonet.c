	...
	while ((skb = skb_peek(&ld_pn->head)) != NULL) {
		...
		if (!room) {
			if (ld_pn->nb_try_to_tx++ > 40) {
				ld_pn->link_up = false;
				/* Flush TX queue */
				while ((skb = skb_dequeue(&ld_pn->head)) \
						!= NULL) {
...
static void ld_phonet_ldisc_initiate_transfer(struct ld_phonet *ld_pn,
	...
	int count)
{
	...
	while (ld_pn->n_Data_Processed < count) {
		...
		if (count > ISI_MSG_HEADER_SIZE) {
			...
			ph = (struct phonethdr *)(cp + ld_pn->n_Data_Processed \
				+ sizeof(char));
			...
			if (ld_pn->len <= count) {
				...
				__skb_pull(skb, 1);  /* we remove media id */
				...
				ld_pn->n_Remaining_Data = ld_pn->len - (count \
					- ld_pn->n_Data_Sent);
...
(struct tty_struct *tty, const unsigned char *cp, char *fp, int count)
{
	...
	/* Whenever we receive a new USB frame Data Processed should be reset */
	ld_pn->n_Data_Processed = 0;
...
{
	return tty_register_ldisc(N_PHONET, &ld_phonet_ldisc);
}
