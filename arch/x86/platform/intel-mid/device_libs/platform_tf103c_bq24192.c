...
static int __init bq24192_platform_init(void)
{
	if (msic_battery_check())
       ...
       i2c_adap = i2c_get_adapter(BQ24192_I2C_ADAPTER);
       if (!i2c_adap)  {
		printk("[%s] Cannot get i2c adapter %d\n", __func__, BQ24192_I2C_ADAPTER);
		goto err;
       }

       i2c_client = i2c_new_device(i2c_adap, &bq24192_i2c_board_info);
       if (!i2c_client) {
		printk("[%s] Unable to add I2C device for 0x%x\n", __func__,
		bq24192_i2c_board_info.addr);
		goto err;
       }
err:    
        printk("=====================  %s \n",__func__);
