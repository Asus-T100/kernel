...
//2M-0.3M
struct intel_v4l2_subdev_id tf103ce_vl42_ids_1[] = {
...
struct intel_v4l2_subdev_id *current_tab = tf103ce_vl42_ids_1;
static struct camera_device_table tf103ce_cam_table[] = {
        {
...
struct intel_v4l2_subdev_id *get_table_entry(char *sensor_name)
{
	...
	entry_num = ARRAY_SIZE(tf103ce_vl42_ids_1);
	table = tf103ce_vl42_ids_1;
...
{
	current_tab = tf103ce_vl42_ids_1;
	return &tf103ce_vl42_ids_1;
}

struct camera_device_table *get_camera_table(void)
{
	printk("%s is called",__func__);
	return &tf103ce_cam_table;
}

int get_camera_tablesize(void)
{
	int size = ARRAY_SIZE(tf103ce_cam_table);
	printk("%s is called",__func__);
	return size;
}
