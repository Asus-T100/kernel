...
#if defined(AMPDU_VO_ENABLE) || defined(BCM4334X_MCC_ENABLE)
#include <proto/802.1d.h>
#endif /* AMPDU_VO_ENABLE || BCM4334X_MCC_ENABLE */
...
uint dhd_eventmsgs_ver = EVENTMSGS_VER;
module_param(dhd_eventmsgs_ver, uint, 0644);
...
dhd_sendpkt(dhd_pub_t *dhdp, int ifidx, void *pktbuf)
{
	...
	if (dhdp->mchan_flowctrl != 0x3) {
		...
		dhd_tcpack_suppress_set(dhdp, TCPACK_SUP_DELAYTX);
...
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
	...
	if (dhd_get_fw_mode(dhdinfo) == DHD_FLAG_MFG_MODE)
		DHD_ERROR(("Don't clear mfg firmware path\n"));
	else
		firmware_path[0] = '\0';
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	eventmask_msg->ver = dhd_eventmsgs_ver;
