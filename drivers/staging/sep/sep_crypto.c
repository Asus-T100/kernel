 ...
 */
void crypto_sep_dump_message(struct sep_system_ctx *sctx)
{
	u32 *p;
	u32 *i;
	int count;
...
/**
 * sep_wait_for_sctx
 * @sctx: pointer to struct sep system_ctx
 * @returns: 0 if we got sctx, -1 otherwise
 */
int sep_wait_for_sctx(struct sep_system_ctx *sctx)
{
	...
	pr_debug("wait for sctx\n");
	while ((time_before(jiffies, end_time)) && (result == -1)) {
		pr_debug("attempt to get sctx\n");
		schedule();
		if (!test_and_set_bit(0, &sctx->using_sctx) == 0)
			result = 0;
	}

	if (result == -1)
		pr_debug("could not get sctx\n");
	return result;
}
 ...
 */
u32 sep_start_msg(struct sep_system_ctx *sctx)
{
	u32 *word_ptr;
 ...
 */
void sep_end_msg(struct sep_system_ctx *sctx, u32 msg_offset)
{
	u32 *word_ptr;
 ...
 */
u32 sep_start_inbound_msg(struct sep_system_ctx *sctx, u32 *msg_offset)
{
	u32 *word_ptr;
	u32 token;
	u32 error = SEP_OK;
 ...
 */
void sep_write_msg(struct sep_system_ctx *sctx, void *in_addr,
 ...
 */
void sep_make_header(struct sep_system_ctx *sctx, u32 *msg_offset, u32 op_code)
{
	u32 *word_ptr;
 ...
 */
void sep_read_msg(struct sep_system_ctx *sctx, void *in_addr,
	u32 size, u32 max_size, u32 *msg_offset, u32 byte_array)
{
	u32 *word_ptr;
	void *void_ptr;
 ...
 */
u32 sep_verify_op(struct sep_system_ctx *sctx, u32 op_code, u32 *msg_offset)
{
	u32 error;
	u32 in_ary[2];
 ...
 * of the context
 */
void sep_read_context(struct sep_system_ctx *sctx, u32 *msg_offset,
	void *dst, u32 len)
{
 ...
 * of the context
 */
void sep_write_context(struct sep_system_ctx *sctx, u32 *msg_offset,
	void *src, u32 len)
{
	u32 max_length = ((len + 3) / sizeof(u32)) * sizeof(u32);
 ...
 */
void sep_clear_out(struct sep_system_ctx *sctx)
{
	if (sctx->src_sg_hold) {
...
void sep_crypto_release(struct sep_system_ctx *sctx, u32 error)
{
	...
	/**
	 * If we are through with this transaction (not return -EINPROGRESS)
	 * we need to clear the done with sctx bit. This is the bit that
	 * prevents another transaction from touching this sctx. This
	 * action must be the last ever to touch the sctx because the
	 * the sctx may dissapear from under us due to it being kfree'd
	 * by the kernel crypto infrastructure
	 */
	if (error != -EINPROGRESS)
		clear_bit(0, &sctx->using_sctx);
}
 ...
 *	were problems
 */
int sep_crypto_take_sep(struct sep_system_ctx *sctx)
{
	struct sep_device *sep = sctx->sep_used;
	int result;
	...
	set_bit(SEP_LEGACY_SENDMSG_DONE_OFFSET, &sctx->call_status.status);
...
u32 hash_update_post_op(struct sep_device *sep)
{
	...
	if (sctx->sep_used->current_hash_stage == HASH_FINUP_DATA) {

		/* first reset stage to HASH_FINUP_FINISH */
		sctx->sep_used->current_hash_stage = HASH_FINUP_FINISH;

		/* now enqueue the finish operation */
		spin_lock_irq(&queue_lock);
		u32_error = crypto_enqueue_request(&sep_queue,
			&sctx->sep_used->current_hash_req->base);
		spin_unlock_irq(&queue_lock);
		...
		/* schedule the data send */
		u32_error = sep_submit_work(sctx->sep_used->workqueue,
	...
	/* Signal to crypto infrastructure and clear out */
	dev_dbg(&sctx->sep_used->pdev->dev, "hash update post op done\n");
	sep_crypto_release(sctx, 0);
	return 0;
}
...
static void sep_dequeuer(void *data)
{
	...
	if (async_req->tfm->__crt_alg->cra_type !=
		...
		if (sctx->current_hash_stage == HASH_INIT) {
			...
			return;
		} else if (sctx->current_hash_stage == HASH_FINUP_DATA) {
			pr_debug("sep crypto queue hash digest\n");
			sep_hash_update((void *)hash_req);
			return;
		} else if (sctx->current_hash_stage == HASH_FINUP_FINISH) {
			pr_debug("sep crypto queue hash digest\n");
			sep_hash_final((void *)hash_req);
			return;
		} else {
...
static int sep_sha1_finup(struct ahash_request *req)
{
	...
	sctx->current_cypher_req = NULL;
	ctx->hash_opmode = SEP_HASH_SHA1;
	sctx->current_hash_stage = HASH_FINUP_DATA;
	...
	if (error) {
		dev_warn(&sctx->sep_used->pdev->dev,
			"sha1 finup cannot submit queue\n");
		sep_crypto_release(sctx, -EINVAL);
		return -EINVAL;
	}
	return -EINPROGRESS;
...
static int sep_sha224_finup(struct ahash_request *req)
{
	...
	if (error) {
		dev_warn(&sctx->sep_used->pdev->dev,
			"sha256 finup cannot submit queue\n");
		sep_crypto_release(sctx, -EINVAL);
		return -EINVAL;
	}
	return -EINPROGRESS;
