...
extern IMG_INT strlen(const IMG_CHAR * pszString);
#endif
...
PDBG_LASTFRAME_BUFFER FindLFBufFromList(PDBG_STREAM psStream,
...
static IMG_UINT32 DBGDrivWritePersist(PDBG_STREAM psMainStream,
				      IMG_UINT8 * pui8InBuf,
...
/*!
 ...
 @return	none
 */
IMG_VOID IMG_CALLCONV ExtDBGDrivSetConnectNotifier(DBGKM_CONNECT_NOTIFIER
						   fn_notifier)
{
					    ...
					    IMG_BOOL bResetStream)
{
	...
	if (g_fnDBGKMNotifier.pfnConnectNotifier) {
		...
		PVR_DPF((PVR_DBG_ERROR,
			 "pfnConnectNotifier not initialised.\n"));
	}
 ...
 */
IMG_BOOL IMG_CALLCONV ExtDBGDrivIsCaptureFrame(PDBG_STREAM psStream,
 ...
 */
IMG_UINT32 IMG_CALLCONV ExtDBGDrivWritePersist(PDBG_STREAM psStream,
					       IMG_UINT8 * pui8InBuf,
					       IMG_UINT32 ui32InBuffSize,
					       IMG_UINT32 ui32Level)
{
	...
	ui32Ret =
	    DBGDrivWritePersist(psStream, pui8InBuf, ui32InBuffSize, ui32Level);
	if (ui32Ret == 0xFFFFFFFFU) {
		PVR_DPF((PVR_DBG_ERROR,
			 "An error occurred in DBGDrivWritePersist."));
	}
...
IMG_UINT32 AtoI(IMG_CHAR * szIn)
{
	...
	for (iPos = iLen - 1; iPos >= 0; iPos--) {
		...
		{
			bc -= 'a' - 0xa;
		} else if ((bc >= 'A') && (bc <= 'F') && ui32Base == 16)	//handle upper case A-F
		{
			bc -= 'A' - 0xa;
		} else if ((bc >= '0') && (bc <= '9'))	//if char out of range, return 0
		{
			bc -= '0';
		} else
			return (0);
...
/*!****************************************************************************
 ...
 @return	none
*****************************************************************************/
static void Write(PDBG_STREAM psStream, IMG_PUINT8 pui8Data,
		  IMG_UINT32 ui32InBuffSize)
{
	/*
	...
	if ((psStream->ui32WPtr + ui32InBuffSize) > psStream->ui32Size) {
		...
		HostMemCopy((IMG_PVOID)
			    ((IMG_UINTPTR_T) psStream->pvBase +
			     psStream->ui32WPtr), (IMG_PVOID) pui8Data, ui32B1);

		/* Copy second block to start of buffer */
		HostMemCopy(psStream->pvBase,
			    (IMG_PVOID) (pui8Data + ui32B1), ui32B2);

		/* Set pointer to be the new end point */
		psStream->ui32WPtr = ui32B2;
	} else {		/* Can fit block in single chunk */
		HostMemCopy((IMG_PVOID)
			    ((IMG_UINTPTR_T) psStream->pvBase +
			     psStream->ui32WPtr), (IMG_PVOID) pui8Data,
			    ui32InBuffSize);
...
/*!****************************************************************************
 ...
 @return	bytes copied
*****************************************************************************/
static IMG_UINT32 WriteExpandingBuffer(PDBG_STREAM psStream,
				       IMG_UINT8 * pui8InBuf,
				       IMG_UINT32 ui32InBuffSize)
{
	IMG_UINT ui32Space;
	...
	if (psStream->psCtrl->ui32Flags & DEBUG_FLAGS_NO_BUF_EXPANDSION) {
		...
		if ((ui32Space < 32) || (ui32Space <= (ui32InBuffSize + 4))) {
			...
			PVR_DPF((PVR_DBGDRIV_MESSAGE,
				 "Expanding buffer size = %x, new size = %x",
					   ...
					   IMG_UINT32 ui32Size)
{
	...
	if ((!psStream) ||
	    (!psInitStream) || (!psLFBuffer) || (!psLFDeinitBuffer) || (!psCtrl)
	    ) {
		PVR_DPF((PVR_DBG_ERROR,
	...
	if (!pvBase) {
		PVR_DPF((PVR_DBG_ERROR,
			 "DBGDriv: Couldn't alloc InitStream buffer\n\r"));

		if ((psStream->psCtrl->
		     ui32Flags & DEBUG_FLAGS_USE_NONPAGED_MEM) != 0) {
...
#ifdef WIN9X
	if (psStream->hTimeout) {
		Cancel_Time_Out(psStream->hTimeout);
					 ...
					 IMG_BOOL bResetStream)
{
	...
	if (bResetStream && psStream) {
		...
		if (psStream->psCtrl->bInitPhaseComplete == IMG_FALSE) {
			if (psStream->psCtrl->
			    ui32Flags & DEBUG_FLAGS_TEXTSTREAM) {
...

		{
			...
			psStream->psInitStream->ui32InitPhaseWOff =
			    psStream->psInitStream->ui32WPtr;
			PVR_DPF((PVR_DBGDRIV_MESSAGE,
				 "Set %s client marker bo %x, total bw %x",
				 psStream->szName,
				 psStream->psInitStream->ui32InitPhaseWOff,
				 psStream->psInitStream->ui32DataWritten));
		}
...
static void IMG_CALLCONV DBGDrivInvalidateStream(PDBG_STREAM psStream)
{
	...
	PVR_DPF((PVR_DBG_ERROR,
		 "DBGDrivInvalidateStream: An error occurred for stream %s\r\n",
		 psStream->szName));
	...
	if (ui32Space > 0) {
		...
		PVR_DPF((PVR_DBG_ERROR,
			 "DBGDrivInvalidateStream: Buffer full."));
	}
					     ...
					     IMG_UINT32 ui32Level)
{
	...
	if (!psStream->psCtrl->bInitPhaseComplete) {
		...
		if ((psStream->psCtrl->ui32Flags & DEBUG_FLAGS_ENABLESAMPLE) ==
		    0) {
			return (0);
		}
	} else {
					   ...
					   IMG_UINT32 ui32Level)
{
	 ...
	 */
	if (!(((psStream->psCtrl->ui32OutMode & DEBUG_OUTMODE_STREAMENABLE) !=
	       0)
				     ...
				     IMG_UINT32 ui32Level)
{
	...
	if (psMainStream->psCtrl->ui32CapMode & DEBUG_CAPMODE_FRAMED) {
		if ((psMainStream->psCtrl->
		     ui32Flags & DEBUG_FLAGS_ENABLESAMPLE) == 0) {
			/* throw away non-capturing data */
			return (ui32InBuffSize);
		}
	} else if (psMainStream->psCtrl->ui32CapMode == DEBUG_CAPMODE_HOTKEY) {
		if ((psMainStream->psCtrl->ui32Current != g_ui32HotKeyFrame)
		    || (g_bHotKeyPressed == IMG_FALSE)) {
			/* throw away non-capturing data */
			return (ui32InBuffSize);
		}
	...
	PVR_DPF((PVR_DBGDRIV_MESSAGE, "Recv %d b for %s: Roff = %x, WOff = %x",
		 ui32InBuffSize,
		 psStream->szName, psStream->ui32RPtr, psStream->ui32WPtr));
				      ...
				      IMG_UINT32 ui32Level)
{
	...
	if (psStream->bCircularAllowed == IMG_TRUE) {
		PVR_DPF((PVR_DBG_WARNING,
			 "DBGDrivWritePersist: Init phase is a circular buffer, some data may be lost"));
	}

	PVR_DPF((PVR_DBGDRIV_MESSAGE,
		 "Append %x b to %s: Roff = %x, WOff = %x [bw = %x]",
		 ui32InBuffSize, psStream->szName, psStream->ui32RPtr,
		 psStream->ui32WPtr, psStream->ui32DataWritten));
				      ...
				      IMG_UINT32 ui32Level)
{
	...
	PVR_DPF((PVR_DBGDRIV_MESSAGE,
		 "Recv(exp) %d b for %s: Roff = %x, WOff = %x", ui32InBuffSize,
		 psStream->szName, psStream->ui32RPtr, psStream->ui32WPtr));
				    ...
				    IMG_UINT8 * pui8OutBuf)
{
	 ...
	 */
	if ((psStream->ui32RPtr + ui32Data) > psStream->ui32Size) {	/* Calc block 1 and block 2 sizes */
...
#ifdef WIN9X
	asdsds if (psStream->psCtrl->ui32OutMode & DEBUG_OUTMODE_ASYNC) {
		if (!psStream->hTimeout) {
			psStream->hTimeout =
			    Set_Global_Time_Out(AsyncWriteString, 1,
						(IMG_UINT32) psStream);
		}
	} else {
		if (psStream->hTimeout) {
...
void IMG_CALLCONV DBGDrivSetFrame(PDBG_STREAM psStream, IMG_UINT32 ui32Frame)
{
	...
	if ((ui32Frame >= psStream->psCtrl->ui32Start) &&
	    ...
	    (((ui32Frame -
	       psStream->psCtrl->ui32Start) %
	      psStream->psCtrl->ui32SampleRate) == 0)) {
	...
	if (g_bHotkeyMiddump) {
		if ((ui32Frame >= g_ui32HotkeyMiddumpStart) &&
			...
			psStream->psCtrl->ui32Flags &=
			    ~DEBUG_FLAGS_ENABLESAMPLE;
			if (psStream->psCtrl->ui32Current >
			    g_ui32HotkeyMiddumpEnd) {
				g_bHotkeyMiddump = IMG_FALSE;
			}
		}
	}
	...
	if (g_bHotKeyRegistered) {
		...
		if (((psStream->psCtrl->ui32CapMode & DEBUG_CAPMODE_FRAMED) !=
		     0)
		    && ((psStream->psCtrl->ui32CapMode & DEBUG_CAPMODE_HOTKEY)
			!= 0)) {
			if (!g_bHotkeyMiddump) {
				/* Turn on */
				g_ui32HotkeyMiddumpStart =
...
*****************************************************************************/
IMG_BOOL IMG_CALLCONV DBGDrivIsCaptureFrame(PDBG_STREAM psStream,
					    IMG_BOOL bCheckPreviousFrame)
{
	...
	if (psStream->psCtrl->ui32CapMode & DEBUG_CAPMODE_FRAMED) {
		...
		if (g_bHotkeyMiddump) {
			...
			if ((psStream->psCtrl->ui32Current >=
			     (psStream->psCtrl->ui32Start - ui32FrameShift))
			    && (psStream->psCtrl->ui32Current <=
				(psStream->psCtrl->ui32End - ui32FrameShift))
			    &&
			    ((((psStream->psCtrl->ui32Current +
				ui32FrameShift) -
			       psStream->psCtrl->ui32Start) %
...
/*!****************************************************************************
 ...
 @return	nothing
*****************************************************************************/
IMG_VOID IMG_CALLCONV DBGDrivSetClientMarker(PDBG_STREAM psStream,
					     IMG_UINT32 ui32Marker)
{
	/*
	   Validate buffer
					     ...
					     IMG_UINT32 ui32StreamOffset)
{
	...
	PVR_DPF((PVR_DBGDRIV_MESSAGE, "DBGDrivSetStreamOffset: %s set to %x b",
		 psStream->szName, ui32StreamOffset));
				       ...
				       IMG_UINT32 ui32Flags)
{
	...
	if ((psStream->psCtrl->ui32CapMode & DEBUG_CAPMODE_FRAMED) != 0) {
		...
		if (psStream->psCtrl->bInitPhaseComplete &&
		    ((psStream->psCtrl->ui32Current != g_ui32HotKeyFrame)
		     || (g_bHotKeyPressed == IMG_FALSE))) {
			/* throw away non-capturing data */
			return (ui32InBuffSize);
		}
				      ...
				      IMG_UINT8 * pui8OutBuf)
{
	...
	if (ui32OutBuffSize < ui32TotalSize) {
		PVR_DPF((PVR_DBG_ERROR,
			 "DBGDrivReadLF: Not enough space (%d) to copy LF and LF deinit buffers(%d).",
			 ui32OutBuffSize, ui32TotalSize));
		/* don't copy anything */
		ui32TotalSize = 0;
	} else {
		/*
		   Copy the data into the buffer
		 */
		HostMemCopy(pui8OutBuf, psLFBuffer->ui8Buffer,
			    psLFBuffer->ui32BufLen);

		HostMemCopy((pui8OutBuf + psLFBuffer->ui32BufLen),
			    psLFDeinitBuffer->ui8Buffer,
			    psLFDeinitBuffer->ui32BufLen);
	}
	...
	if (psStream->bCircularAllowed) {
		...
		if (psStream->ui32RPtr <= psStream->ui32WPtr) {
			...
			HostMemCopy(pvNewBuf,
				    (IMG_VOID *) ((IMG_UINTPTR_T) psStream->
						  pvBase + psStream->ui32RPtr),
			 ...
			 */
			ui32FirstCopySize =
			    psStream->ui32Size - psStream->ui32RPtr;
			...
			/*
			   Now second half 
			 */
			HostMemCopy((IMG_VOID *) ((IMG_UINTPTR_T) pvNewBuf +
						  ui32FirstCopySize),
