...
#endif
{
	...
	if (atomic_dec_and_test(&sDriverIsShutdown)) {
		/*
		 * Take the bridge mutex to stop processes trying to use the
		 * driver after it has been shutdown. The fact that the mutex
		 * isn't released again is deliberate. The atomic variable,
		 * sDriverIsShutdown, prevents this code being run more than
		 * once, where multiple devices are being managed.
		 */
...
#if defined(SUPPORT_DRI_DRM) && defined(PVR_SECURE_DRM_AUTH_EXPORT)
	psEnvConnection =
	    PVRSRVConnectionPrivateData(psPrivateData->pvConnectionData);
	psPrivateData->psDRMFile = pFile;

	list_add_tail(&psPrivateData->sDRMAuthListItem,
		      &psEnvConnection->sDRMAuthListHead);
#endif
