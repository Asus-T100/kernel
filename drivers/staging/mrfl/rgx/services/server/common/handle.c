...
#endif
static INLINE IMG_VOID InitChildEntry(struct sHandle *psHandle)
{
	HandleListInit(HANDLE_PTR_TO_INDEX(psHandle), &psHandle->sSiblings,
		       IMG_NULL);
}
...
#endif
static INLINE IMG_BOOL NoChildren(struct sHandle *psHandle)
{
	PVR_ASSERT(psHandle->sChildren.hParent ==
		   HANDLE_PTR_TO_HANDLE(psHandle));

	return HandleListIsEmpty(HANDLE_PTR_TO_INDEX(psHandle),
				 &psHandle->sChildren);
}
...
#endif
static INLINE IMG_BOOL NoParent(struct sHandle *psHandle)
{
	if (HandleListIsEmpty
	    (HANDLE_PTR_TO_INDEX(psHandle), &psHandle->sSiblings)) {
				    ...
				    IMG_UINT32 ui32ParentIndex)
{
	...
	PVR_ASSERT(LIST_PTR_FROM_INDEX_AND_OFFSET
		   (psBase, ui32ParentIndex, ui32ParentIndex, uiParentOffset,
		    uiParentOffset)->hParent ==
		   INDEX_TO_HANDLE(ui32ParentIndex));
...
static INLINE
    IMG_VOID UnlinkFromParent(PVRSRV_HANDLE_BASE * psBase,
			      struct sHandle *psHandle)
{
	HandleListRemove(psBase, HANDLE_PTR_TO_INDEX(psHandle),
				   ...
				   (PVRSRV_HANDLE_BASE *, struct sHandle *))
{
	...
	for (ui32Index = psHead->ui32Next; ui32Index != ui32Parent;) {
		struct sHandle *psHandle =
		    INDEX_TO_HANDLE_STRUCT_PTR(psBase, ui32Index);
...
******************************************************************************/
static
PVRSRV_ERROR ReallocHandleArray(PVRSRV_HANDLE_BASE * psBase,
				IMG_UINT32 ui32NewCount)
{
	...
	if (ui32NewCount != 0) {
		...
		psNewArray =
		    OSAllocMem(HANDLE_ARRAY_SIZE(ui32NewCount) *
			       sizeof(struct sHandleIndex));
		if (psNewArray == IMG_NULL) {
			PVR_DPF((PVR_DBG_ERROR,
				 "ReallocHandleArray: Couldn't allocate new handle array"));
			eReturn = PVRSRV_ERROR_OUT_OF_MEMORY;
			goto error;
		}

		if (ui32OldCount != 0) {
			OSMemCopy(psNewArray, psOldArray,
				  HANDLE_ARRAY_SIZE(MIN
						    (ui32NewCount,
						     ui32OldCount)) *
				  sizeof(struct sHandleIndex));
		}
	 ...
	 */
	for (ui32Index = ui32NewCount; ui32Index < ui32OldCount;
	     ui32Index += HANDLE_BLOCK_SIZE) {
		struct sHandleIndex *psIndex =
		    INDEX_TO_INDEX_STRUCT_PTR(psOldArray, ui32Index);

		OSFreeMem(psIndex->psHandle);
	}
	...
	for (ui32Index = ui32OldCount; ui32Index < ui32NewCount;
		...
		struct sHandleIndex *psIndex =
		    INDEX_TO_INDEX_STRUCT_PTR(psNewArray, ui32Index);

		psIndex->psHandle =
		    OSAllocMem(sizeof(struct sHandle) * HANDLE_BLOCK_SIZE);
		if (psIndex->psHandle == IMG_NULL) {
			PVR_DPF((PVR_DBG_ERROR,
				 "ReallocHandleArray: Couldn't allocate handle structures"));
			eReturn = PVRSRV_ERROR_OUT_OF_MEMORY;
		} else {
			...
			for (ui32SubIndex = 0; ui32SubIndex < HANDLE_BLOCK_SIZE;
			     ui32SubIndex++) {
				struct sHandle *psHandle =
				    psIndex->psHandle + ui32SubIndex;

				psHandle->ui32Index = ui32SubIndex + ui32Index;
				psHandle->eType = PVRSRV_HANDLE_TYPE_NONE;
				psHandle->eInternalFlag =
				    INTERNAL_HANDLE_FLAG_NONE;
	...
	if (ui32NewCount > DEBUG_MAX_HANDLE_COUNT) {
		PVR_DPF((PVR_DBG_ERROR,
			 "ReallocHandleArray: Max handle count (%u) reached",
			 DEBUG_MAX_HANDLE_COUNT));
		eReturn = PVRSRV_ERROR_OUT_OF_MEMORY;
		goto error;
	}
#endif
	...
	if (ui32NewCount > ui32OldCount) {
		...
		PVR_ASSERT(psBase->ui32FreeHandCount +
			   (ui32NewCount - ui32OldCount) >
		...
		if (psBase->ui32FirstFreeIndex == 0) {
			...
			if (!psBase->bPurgingEnabled) {
				PVR_ASSERT(psBase->ui32LastFreeIndexPlusOne !=
					   0);
				PVR_ASSERT(INDEX_TO_HANDLE_STRUCT_PTR
					   (psBase,
					    psBase->ui32LastFreeIndexPlusOne -
					    1)->ui32NextIndexPlusOne == 0);

				INDEX_TO_HANDLE_STRUCT_PTR(psBase,
							   psBase->
							   ui32LastFreeIndexPlusOne
							   -
							   1)->
				    ui32NextIndexPlusOne = ui32OldCount + 1;
			}
		}
		...
		PVR_ASSERT(ui32NewCount == 0
			   || psBase->ui32FirstFreeIndex <= ui32NewCount);
		PVR_ASSERT(psBase->ui32FreeHandCount -
			   (ui32OldCount - ui32NewCount) <
			   psBase->ui32FreeHandCount);
			       ...
			       struct sHandle *psHandle)
{
	...
	PVR_ASSERT(INDEX_TO_FREE_HAND_BLOCK_COUNT(psBase, ui32Index) <=
		   HANDLE_BLOCK_SIZE);
...
#ifdef DEBUG
	{
		...
		for (ui32BlockedIndex = 0;
		     ui32BlockedIndex < psBase->ui32TotalHandCount;
		     ui32BlockedIndex += HANDLE_BLOCK_SIZE) {
			ui32FreeHandCount +=
			    INDEX_TO_FREE_HAND_BLOCK_COUNT(psBase,
							   ui32BlockedIndex);
		}
...
{
	PVRSRV_ERROR eError;
	IMG_UINT32 ui32DeltaAdjusted =
	    ROUND_UP_TO_MULTIPLE_OF_BLOCK_SIZE(ui32Delta);
				...
				IMG_HANDLE hParent)
{
	...
	if (!psBase->bPurgingEnabled) {
		 ...
		 */
		PVR_ASSERT((psBase->ui32FirstFreeIndex % HANDLE_BLOCK_SIZE) ==
			   0);

		for (ui32BlockedIndex =
		     ROUND_DOWN_TO_MULTIPLE_OF_BLOCK_SIZE(psBase->
							  ui32FirstFreeIndex);
		     ui32BlockedIndex < psBase->ui32TotalHandCount;
		     ui32BlockedIndex += HANDLE_BLOCK_SIZE) {
			struct sHandleIndex *psIndex =
			    BASE_AND_INDEX_TO_INDEX_STRUCT_PTR(psBase,
							       ui32BlockedIndex);
			...
			for (ui32NewIndex = ui32BlockedIndex;
			     ui32NewIndex <
			     ui32BlockedIndex + HANDLE_BLOCK_SIZE;
			     ui32NewIndex++) {
				psNewHandle =
	...
	PVR_ASSERT(INDEX_TO_FREE_HAND_BLOCK_COUNT(psBase, ui32NewIndex) <=
		   HANDLE_BLOCK_SIZE);
	...
	if (!TEST_FLAG(eFlag, PVRSRV_HANDLE_ALLOC_FLAG_MULTI)) {
		...
		if (hHandle != IMG_NULL) {
			...
			PVR_ASSERT(hParentKey != IMG_NULL
				   &&
				   ParentHandle(HANDLE_TO_HANDLE_STRUCT_PTR
						     ...
						     psBase, IMG_BOOL bCommit)
{
	...
	while (ui32IndexPlusOne != 0) {
		struct sHandle *psHandle =
		    INDEX_TO_HANDLE_STRUCT_PTR(psBase, ui32IndexPlusOne - 1);
		IMG_UINT32 ui32NextIndexPlusOne =
	...
	if (psBase->ui32TotalHandCountPreBatch != psBase->ui32TotalHandCount) {
		...
		/*
		 * FIXME Is this an error or not? If yes, return an error. If not, delete this.
		 */
		PVR_DPF((PVR_DBG_WARNING,
				...
				IMG_UINT32 ui32MaxHandle)
{
	...
	if (psBase->ui32TotalHandCount != 0) {
		PVR_DPF((PVR_DBG_ERROR,
			 "PVRSRVSetMaxHandle: Limit cannot be set because handles have already been allocated"));

		return PVRSRV_ERROR_INVALID_PARAMS;
	}

	ui32MaxHandleRounded =
	    ROUND_DOWN_TO_MULTIPLE_OF_BLOCK_SIZE(ui32MaxHandle);

	/*
	 * Allow the maximum number of handles to be reduced, but never to
	 * zero.
	 */
	if (ui32MaxHandleRounded != 0
	    && ui32MaxHandleRounded < psBase->ui32MaxIndexPlusOne) {
...
PVRSRV_ERROR PVRSRVPurgeHandles(PVRSRV_HANDLE_BASE * psBase)
{
	...
	for (ui32BlockIndex = INDEX_TO_BLOCK_INDEX(psBase->ui32TotalHandCount);
	     ui32BlockIndex != 0; ui32BlockIndex--) {
		if (psBase->psHandleArray[ui32BlockIndex - 1].
		    ui32FreeHandBlockCount != HANDLE_BLOCK_SIZE) {
			break;
		}
	}
...
PVRSRV_ERROR PVRSRVAllocHandleBase(PVRSRV_HANDLE_BASE ** ppsBase)
{
	...
	psBase = OSAllocMem(sizeof(*psBase));
	if (psBase == IMG_NULL) {
		PVR_DPF((PVR_DBG_ERROR,
			 "PVRSRVAllocHandleBase: Couldn't allocate handle base"));
		return PVRSRV_ERROR_OUT_OF_MEMORY;
	}
