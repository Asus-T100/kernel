...
static void psb_get_mtx_control_from_dash(struct drm_psb_private *dev_priv)
{
	...
	if (count >= 50000)
		PSB_DEBUG_GENERAL
		    ("TOPAZ: timeout in get_mtx_control_from_dash\n");

	/* Save the access control register... */
	msvdx_priv->psb_dash_access_ctrl =
	    PSB_RMSVDX32(MSVDX_MTX_RAM_ACCESS_CONTROL);
...
static void psb_release_mtx_control_from_dash(struct drm_psb_private *dev_priv)
{
	...
	PSB_WMSVDX32(msvdx_priv->psb_dash_access_ctrl,
			  ...
			  int fw_sel)
{
	...
	cmd =
	    PSB_DMAC_VALUE_PERIPH_PARAM(PSB_DMAC_ACC_DEL_0, PSB_DMAC_INCR_OFF,
					PSB_DMAC_BURST_2);
	...
	PSB_WMSVDX32(uCountReg | DMAC_DMAC_COUNT_EN_MASK,
		     REGISTER(DMAC, DMAC_COUNT) + dma_channel);
	...
	if (psb_wait_for_register
	    (dev_priv, REGISTER(MTX_CORE, CR_MTX_SYSC_CDMAS0), 1, 1)) {
		psb_release_mtx_control_from_dash(dev_priv);
		return;
	}
			   ...
			   const struct firmware **raw, uint8_t * name)
{
	...
	rc = ttm_bo_kmap(msvdx_priv->fw, 0, (msvdx_priv->fw)->num_pages,
			 &tmp_kmap);
	if (rc) {
		...
		memcpy(gpu_addr +
		       (((struct msvdx_fw *)ptr)->data_location -
			MSVDX_MTX_DATA_LOCATION),
		       (void *)ptr + sizeof(struct msvdx_fw) +
		       sizeof(uint32_t) * ((struct msvdx_fw *)ptr)->text_size,
		       sizeof(uint32_t) * ((struct msvdx_fw *)ptr)->data_size);

		last_word =
		    (uint32_t *) (gpu_addr + msvdx_priv->mtx_mem_size - 4);
...
int psb_setup_fw(struct drm_device *dev)
{
	...
	PSB_WMSVDX32(DSIABLE_IDLE_GPIO_SIG | DSIABLE_Auto_CLOCK_GATING |
		     RETURN_VDEB_DATA_IN_COMPLETION | DSIABLE_FW_WDT,
		     MSVDX_COMMS_OFFSET_FLAGS);
	PSB_WMSVDX32(0, MSVDX_COMMS_SIGNATURE);
	...
	if (ec_firmware) {
		fw_ptr +=
		    (((sizeof(struct msvdx_fw) +
		       (fw->text_size + fw->data_size) * 4 +
		       0xfff) & ~0xfff) / sizeof(uint32_t));
		fw = (struct msvdx_fw *)fw_ptr;
	}
...
int psb_msvdx_reset(struct drm_psb_private *dev_priv)
{
	...
	if (IS_PENWELL(dev_priv->dev)) {
		...
		for (loop = 0; loop < 50; loop++)
			ret =
			    psb_wait_for_register(dev_priv, MSVDX_MMU_MEM_REQ,
						  0, 0xff);
		if (ret)
			return ret;
	}
			    ...
			    uint32_t *base_addr, unsigned long size)
{
	...
	/*
	   ...
	   RENDEC_A_SIZE);
	 */
	memset(ttm_kmap_obj_virtual(&tmp_kmap, &is_iomem), 0, size);
	ttm_bo_kunmap(&tmp_kmap);
...
{
	int i, offset_start, offset_end, size, full_line_start, full_line_end,
	    extra_line_start, extra_line_end;
	char *src, *dst;
...
static void psb_msvdx_error_concealment(struct work_struct *data)
{
	...
	ret = ttm_bo_kmap(ref_bo,
			  ...
			  (ref_frame->buffer_size +
			   PAGE_SIZE - 1) >> PAGE_SHIFT, &ref_kmap);
	if (ret) {
		printk("MSVDX ERROR CONCEALMENT: ref kmap failed, %d.\n", ret);
		return;
	}
	ref_start = (unsigned char *)ttm_kmap_obj_virtual(&ref_kmap, &is_iomem);
...
int psb_msvdx_init(struct drm_device *dev)
{
	...
	if (!msvdx_priv->vec_local_mem_data) {
		...
		memset(msvdx_priv->vec_local_mem_data, 0,
		       msvdx_priv->vec_local_mem_size);
	}
	...
	if (!msvdx_priv->ccb0) {
		ret = psb_allocate_ccb(dev, &msvdx_priv->ccb0,
				       &msvdx_priv->base_addr0, RENDEC_A_SIZE);
		if (ret) {
	...
	if (!msvdx_priv->ccb1) {
		ret = psb_allocate_ccb(dev, &msvdx_priv->ccb1,
				       &msvdx_priv->base_addr1, RENDEC_B_SIZE);
		if (ret)
			goto err_exit;
	}
	...
	REGIO_WRITE_FIELD(cmd, MSVDX_RENDEC_CONTROL1, RENDEC_BURST_SIZE_W, 1);
	REGIO_WRITE_FIELD(cmd, MSVDX_RENDEC_CONTROL1, RENDEC_BURST_SIZE_R, 1);
	REGIO_WRITE_FIELD(cmd, MSVDX_RENDEC_CONTROL1,
	...
	if (0) {
		...
		MEMIO_WRITE_FIELD(msg_init, FWRK_GENMSG_SIZE, FW_VA_INIT_SIZE);
