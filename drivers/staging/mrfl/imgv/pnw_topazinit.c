...
static int topaz_upload_fw(struct drm_device *dev,
			   enum drm_pnw_topaz_codec codec, uint32_t core_id);
...
static int mtx_dma_write(struct drm_device *dev, uint32_t core);
...
int pnw_error_dump_reg(struct drm_psb_private *dev_priv, int core_id)
{
	...
	TOPAZ_MULTICORE_READ32(TOPAZSC_CR_MULTICORE_CORE_SEL_0, &reg_val);
	...
	*(topaz_priv->topaz_sync_addr) = ~0;
	...
	for (n = 0; n < topaz_priv->topaz_num_cores; n++) {
		...
		MVEA_WRITE32(MVEA_CR_IMG_MVEA_SRST,
			     ...
			     F_ENCODE(1, MVEA_CR_IMG_MVEA_DCF_SOFT_RESET), n);
		...
		MVEA_WRITE32(MVEA_CR_IMG_MVEA_SRST,
			     ...
			     F_ENCODE(0, MVEA_CR_IMG_MVEA_DCF_SOFT_RESET), n);
	}
...
int pnw_topaz_reset(struct drm_psb_private *dev_priv)
{
	...
	for (i = 0; i < topaz_priv->topaz_num_cores; i++) {
		...
		MVEA_WRITE32(MVEA_CR_IMG_MVEA_SRST,
			     ...
			     |
			     F_ENCODE(1, MVEA_CR_IMG_MVEA_CMC_SOFT_RESET) |
			     F_ENCODE(1, MVEA_CR_IMG_MVEA_DCF_SOFT_RESET), i);
		...
		MVEA_WRITE32(MVEA_CR_IMG_MVEA_SRST,
			     ...
			     |
			     F_ENCODE(0, MVEA_CR_IMG_MVEA_CMC_SOFT_RESET) |
			     F_ENCODE(0, MVEA_CR_IMG_MVEA_DCF_SOFT_RESET), i);
	}
...
int pnw_topaz_setup_fw(struct drm_device *dev, enum drm_pnw_topaz_codec codec)
{
	...
	for (; core_id >= 0; core_id--) {
		...
		if (core_id == 0)
			TOPAZ_WRITE32(TOPAZ_CR_TOPAZ_CMD_FIFO_2,
				      F_ENCODE(1, TOPAZ_CR_CMD_FIFO_FLUSH), 0);
		...
		MTX_WRITE32(MTX_CORE_CR_MTX_ENABLE_OFFSET,
			    MTX_CORE_CR_MTX_ENABLE_MTX_ENABLE_MASK, core_id);
	...
	memset((void *)(topaz_priv->topaz_mtx_wb),
	       0, topaz_priv->topaz_num_cores * MTX_WRITEBACK_DATASIZE_ROUND);
	...
	for (core_id = 0; core_id < topaz_priv->topaz_num_cores; core_id++) {
		MVEA_WRITE32(MVEA_CR_BUFFER_SIDEBAND,
			     F_ENCODE(CACHE_ONLY,
				      MVEA_CR_ABOVE_PARAM_OUT_SBAND) |
			     F_ENCODE(CACHE_ONLY,
				      MVEA_CR_BELOW_PARAM_OUT_SBAND) |
			     F_ENCODE(MEM_AND_CACHE,
				      MVEA_CR_ABOVE_PIX_OUT_SBAND) |
			     F_ENCODE(MEM_AND_CACHE,
				      MVEA_CR_RECON_SBAND) |
			     F_ENCODE(CACHE_ONLY,
				...
				MVEA_CR_ABOVE_PARAM_IN_SBAND)
			     ...
			     F_ENCODE(MEMORY_ONLY,
				      MVEA_CR_CURR_PARAM_SBAND) |
			     F_ENCODE(CACHE_ONLY,
				      MVEA_CR_ABOVE_PIX_IN_SBAND) |
		    ...
		    uint32_t core_id)
{
	...
	MTX_WRITE32(MTX_CORE_CR_MTX_SOFT_RESET_OFFSET,
		    MTX_CORE_CR_MTX_SOFT_RESET_MTX_RESET_MASK, core_id);
	...
	/* #.# fill the dst addr */
	MTX_WRITE32(MTX_CR_MTX_SYSC_CDMAA, data_location, core_id);
	MTX_WRITE32(MTX_CR_MTX_SYSC_CDMAC,
		    F_ENCODE(4, MTX_BURSTSIZE) |
		    F_ENCODE(0, MTX_RNW) |
		    F_ENCODE(1, MTX_ENABLE) |
...
void topaz_mtx_upload_by_register(struct drm_device *dev, uint32_t mtx_mem,
				  ...
				  struct ttm_buffer_object *buf, uint32_t core)
{
	struct drm_psb_private *dev_priv = dev->dev_private;
	uint32_t *buf_p;
	...
	bank_ram_size = ui32Size + (bank_size *
				    (F_EXTRACT
				     (debug_reg, TOPAZ_CR_MTX_RAM_BANKS) - 1));
	...
	pnw_topaz_wait_for_register(dev_priv, REG_START_TOPAZ_MTX_HOST(core)
		    ...
		    uint32_t core_id)
{
	...
	topaz_mtx_upload_by_register(dev, MTX_CORE_CODE_MEM, 0,
	...
	topaz_mtx_upload_by_register(dev, MTX_CORE_DATA_MEM,
				     data_location - MTX_DMA_MEMORY_BASE,
				     data_size, cur_codec_fw->data, core_id);
...
void pnw_topaz_mmu_hwsetup(struct drm_psb_private *dev_priv, uint32_t core_id)
{
	...
	TOPAZ_WRITE32(TOPAZ_CR_MMU_CONTROL0, F_ENCODE(0, TOPAZ_CR_MMU_BYPASS)
...
int pnw_topaz_restore_mtx_state(struct drm_device *dev)
{
	...
	for (; core_id >= 0; core_id--) {
		...
		TOPAZ_WRITE32(TOPAZ_CR_IMG_TOPAZ_SRST,
			      ...
			      F_ENCODE(1, TOPAZ_CR_IMG_TOPAZ_VLC_SOFT_RESET),
			      core_id);
		TOPAZ_WRITE32(TOPAZ_CR_IMG_TOPAZ_SRST, 0, core_id);

		topaz_set_mtx_target(dev_priv, core_id, 0);
		MTX_WRITE32(MTX_CORE_CR_MTX_SOFT_RESET_OFFSET,
			    MTX_CORE_CR_MTX_SOFT_RESET_MTX_RESET_MASK, core_id);
		MTX_WRITE32(MTX_CORE_CR_MTX_SOFT_RESET_OFFSET, 0, core_id);
	...
	for (; core_id >= 0; core_id--) {
		...
		topaz_read_core_reg(dev_priv, core_id, TOPAZ_MTX_PC, &i);
...
int pnw_topaz_save_mtx_state(struct drm_device *dev)
{
	...
	for (core = topaz_priv->topaz_num_cores - 1; core >= 0; core--) {
		...
		COMMS_READ32(TOPAZ_COMMS_CR_STAT_1(0), mtx_reg_state++, core);
		COMMS_READ32(TOPAZ_COMMS_CR_STAT_0(0), mtx_reg_state++, core);
		...
		COMMS_READ32(TOPAZ_COMMS_CR_CMD_DATA_ADDR(0),
			     mtx_reg_state++, core);
		COMMS_READ32(TOPAZ_COMMS_CR_CMD_WORD(0), mtx_reg_state++, core);

		/* save mtx data memory */
		if (0 == core) {
			/*master core */
			cur_codec_fw =
			    &topaz_priv->topaz_fw[topaz_priv->topaz_cur_codec *
						  2];
		} else {
			cur_codec_fw =
			    &topaz_priv->topaz_fw[topaz_priv->topaz_cur_codec *
		...
		if (0 != mtx_dma_read(dev, core, data_location, data_size)) {
		 ...
		 uint32_t source_addr, uint32_t size)
{
	...
	MTX_WRITE32(MTX_CR_MTX_SYSC_CDMAC,
		    F_ENCODE(4, MTX_BURSTSIZE) |
		    F_ENCODE(1, MTX_RNW) |
		    F_ENCODE(1, MTX_ENABLE) | F_ENCODE(size, MTX_LENGTH), core);
	...
	if (0 != topaz_dma_transfer(dev_priv, 0, target->offset, 0,
				    MTX_CR_MTX_SYSC_CDMAT, size, core, 1)) {
...
int mtx_dma_write(struct drm_device *dev, uint32_t core_id)
{
	...
	if (core_id == 0)	/*for master core */
		cur_codec_fw =
		    &topaz_priv->topaz_fw[topaz_priv->topaz_cur_codec * 2];
	else
		cur_codec_fw =
		    &topaz_priv->topaz_fw[topaz_priv->topaz_cur_codec * 2 + 1];
