...
static int psb_msvdx_map_command(struct drm_device *dev,
				 ...
				 unsigned long cmd_size, void **msvdx_cmd,
				 uint32_t sequence, int copy_cmd)
{
	struct drm_psb_private *dev_priv = dev->dev_private;
	struct msvdx_private *msvdx_priv = dev_priv->msvdx_private;
	int ret = 0;
	...
	while (cmd_size_remaining > 0) {
		...
		case VA_MSGID_DEBLOCK_MFLD:{
				...
				msvdx_mmu_invalid =
				    atomic_cmpxchg(&dev_priv->msvdx_mmu_invaldc,
						   1, 0);
				if (msvdx_mmu_invalid == 1) {
					uint32_t flags;
		...
		case VA_MSGID_DEBLOCK:
			...
			/* PTD */
			MEMIO_WRITE_FIELD(cmd, FW_DXVA_DEBLOCK_MMUPTD, mmu_ptd);

			/* printk("Got deblock msg\n"); */
			/* Deblock message is followed by 32 */
			/* bytes of deblock params */
			msvdx_deblock =
			    kmalloc(sizeof(struct psb_msvdx_deblock_queue),
				    GFP_KERNEL);
			...
			memcpy(&msvdx_deblock->dbParams, cmd + 16,
			       sizeof(struct DEBLOCKPARAMS));

			ret = ttm_bo_kmap((struct ttm_buffer_object *)
			...
			if (likely(!ret)) {
				msvdx_deblock->dbParams.pPicparams =
				    kmalloc(msvdx_deblock->dbParams.buffer_size,
					    GFP_KERNEL);
		...
		case VA_MSGID_HOST_BE_OPP:
			...
			for (i = 0; i < MAX_DECODE_BUFFERS; i++) {
				...
				if ((first_empty == -1)
				    && (msvdx_priv->frame_info[i].handle == 0))
					first_empty = i;
			}
			...
			if (!current_frame) {
				...
				current_frame =
				    &(msvdx_priv->frame_info[first_empty]);
			}
			...
			current_frame->picture_width_mb =
			    oppParam->picture_width_mb;
		     ...
		     struct psb_ttm_fence_rep *fence_arg)
{
	...
	psb_fence_or_sync(priv, PSB_ENGINE_VIDEO, fence_type,
			  arg->fence_flags, validate_list, fence_arg, &fence);
...
int psb_mtx_send(struct drm_psb_private *dev_priv, const void *msg)
{
	...
	if (widx + msg_num > buf_size) {
		...
		MEMIO_WRITE_FIELD(pad_msg, FWRK_GENMSG_SIZE,
				  (buf_size - widx) << 2);
		MEMIO_WRITE_FIELD(pad_msg, FWRK_GENMSG_ID, FWRK_MSGID_PADDING);
 ...
 loop:				/* just for coding style check */
	...
	case VA_MSGID_CMD_FAILED:{
			...
			if (msg_id == VA_MSGID_CMD_HW_PANIC)
				PSB_DEBUG_MSVDX
				    ("MSVDX_DEBUG: get panic message.\n");
			else
				PSB_DEBUG_MSVDX
				    ("MSVDX_DEBUG: get failed message.\n");
	...
	case VA_MSGID_CMD_COMPLETED:{
			uint32_t fence;
			uint32_t flags =
			    MEMIO_READ_FIELD(buf, FW_VA_CMD_COMPLETED_FLAGS);
	...
	case VA_MSGID_DEBLOCK_REQUIRED:{
			...
			if ((msvdx_priv->deblock_enabled == 1)
				...
				msvdx_deblock =
				    list_first_entry(&msvdx_priv->deblock_queue,
						     struct
						     psb_msvdx_deblock_queue,
						     head);
 ...
 * MSVDX interrupt.
 */
bool psb_msvdx_interrupt(void *pvData)
{
	...
	if (msvdx_stat & MSVDX_INTERRUPT_STATUS_CR_MMU_FAULT_IRQ_MASK) {
		...
		PSB_DEBUG_IRQ("MSVDX: msvdx_stat: 0x%x(MTX)\n", msvdx_stat);
		       ...
		       struct drm_file *file_priv)
{
	...
	case LNC_VIDEO_GETPARAM_RAR_HANDLER_OFFSET:
		...
		offset =
		    (uint32_t) rar_buf.bus_address - dev_priv->rar_region_start;
