			     ...
			     struct drm_file *file_priv)
{
	...
	return ttm_fence_signaled_ioctl(BCVideoGetPriv(file_priv)->tfile, data);
}

int psb_fence_finish_ioctl(struct drm_device *dev, void *data,
			   struct drm_file *file_priv)
{
	return ttm_fence_finish_ioctl(BCVideoGetPriv(file_priv)->tfile, data);
}

int psb_fence_unref_ioctl(struct drm_device *dev, void *data,
			  struct drm_file *file_priv)
{
	return ttm_fence_unref_ioctl(BCVideoGetPriv(file_priv)->tfile, data);
}

int psb_pl_waitidle_ioctl(struct drm_device *dev, void *data,
			  struct drm_file *file_priv)
{
	return ttm_pl_waitidle_ioctl(BCVideoGetPriv(file_priv)->tfile, data);
}

int psb_pl_setstatus_ioctl(struct drm_device *dev, void *data,
			   struct drm_file *file_priv)
{
	return ttm_pl_setstatus_ioctl(BCVideoGetPriv(file_priv)->tfile,
...
int psb_pl_synccpu_ioctl(struct drm_device *dev, void *data,
			 struct drm_file *file_priv)
{
	return ttm_pl_synccpu_ioctl(BCVideoGetPriv(file_priv)->tfile, data);
}

int psb_pl_unref_ioctl(struct drm_device *dev, void *data,
		       struct drm_file *file_priv)
{
	return ttm_pl_unref_ioctl(BCVideoGetPriv(file_priv)->tfile, data);
...
int psb_pl_reference_ioctl(struct drm_device *dev, void *data,
			   struct drm_file *file_priv)
{
	return ttm_pl_reference_ioctl(BCVideoGetPriv(file_priv)->tfile, data);
			...
			struct drm_file *file_priv)
{
	...
	return ttm_pl_create_ioctl(BCVideoGetPriv(file_priv)->tfile,
			   ...
			   struct drm_file *file_priv)
{
	...
	return ttm_pl_ub_create_ioctl(BCVideoGetPriv(file_priv)->tfile,
				      &dev_priv->bdev, &dev_priv->ttm_lock,
				      data);
...
static int psb_ttm_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
...
int psb_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (unlikely(dev_priv->ttm_vm_ops == NULL)) {
		dev_priv->ttm_vm_ops =
		    (struct vm_operations_struct *)vma->vm_ops;
...
*/
int psb_verify_access(struct ttm_buffer_object *bo, struct file *filp)
{
	struct drm_file *file_priv = (struct drm_file *)filp->private_data;
	...
	return ttm_pl_verify_access(bo, BCVideoGetPriv(file_priv)->tfile);
}
...
int psb_ttm_global_init(struct drm_psb_private *dev_priv)
{
	...
	if (ret != 0) {
		...
		drm_global_item_unref((struct drm_global_reference *)
				      global_ref);
		return ret;
	}
			   ...
			   struct drm_file *file_priv)
{
	...
	bo = ttm_buffer_object_lookup(BCVideoGetPriv(file_priv)->tfile,
				      arg->handle);
	if (unlikely(bo == NULL)) {
		printk(KERN_ERR
