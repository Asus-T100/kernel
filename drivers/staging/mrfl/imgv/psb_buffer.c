			     ...
			     struct ttm_mem_type_manager *man)
{
	...
	case TTM_PL_CI:
		man->func = &ttm_bo_manager_func;
		man->flags = TTM_MEMTYPE_FLAG_MAPPABLE | TTM_MEMTYPE_FLAG_FIXED;
...
static int psb_invalidate_caches(struct ttm_bo_device *bdev, uint32_t placement)
{
	return 0;
}
			 ...
			 struct ttm_mem_reg *new_mem)
{
	...
	ret =
	    ttm_bo_mem_space(bo, &placement, &tmp_mem, interruptible, false,
			     no_wait);
	if (ret)
		return ret;
...
static int drm_psb_tbe_unbind(struct ttm_backend *backend)
{
	...
	if (psb_be->mem_type == TTM_PL_TT) {
		uint32_t gatt_p_offset =
		    (psb_be->offset -
		     dev_priv->pg->mmu_gatt_start) >> PAGE_SHIFT;
			    ...
			    struct ttm_mem_reg *bo_mem)
{
	...
	type =
	    (bo_mem->placement & TTM_PL_FLAG_CACHED) ? PSB_MMU_CACHED_MEMORY :
	    0;
