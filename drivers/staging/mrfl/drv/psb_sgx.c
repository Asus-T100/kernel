...
static void psb_unreference_buffers(struct psb_context *context)
{
	...
	list_for_each_entry_safe(entry, next, list, head) {
		vbuf = container_of(entry, struct psb_validate_buffer, base);
		list_del(&entry->head);
...
static int psb_reference_buffers(struct drm_file *file_priv,
				 uint64_t data, struct psb_context *context)
{
	struct psb_validate_buffer *item;
	int ret;
			 ...
			 uint32_t new_fence_class, uint32_t *new_fence_type)
{
	...
	if (unlikely
	    (!(set_val_flags & (PSB_GPU_ACCESS_READ | PSB_GPU_ACCESS_WRITE)))) {
	...
	*new_fence_type = n_fence_type;
	...
	if (old_fence && ((new_fence_class != old_fence->fence_class) ||
			  ((n_fence_type ^ old_fence_type) & old_fence_type))) {
			   ...
			   int no_wait, int interruptible)
{
	...
	align_shift =
	    (reloc->shift & PSB_RELOC_ALSHIFT_MASK) >> PSB_RELOC_ALSHIFT_SHIFT;
			    ...
			    int no_wait, int interruptible)
{
	...
	reloc = (struct drm_psb_reloc *)
	    ((unsigned long)
	     ttm_kmap_obj_virtual(&reloc_kmap, &reloc_is_iomem) + reloc_offset);
		       ...
		       struct ttm_fence_object **fence_p)
{
	...
	if (!(fence_flags & DRM_PSB_FENCE_NO_USER)) {
		...
		ret = ttm_ref_object_base_unref(tfile, handle, ttm_fence_type);
		BUG_ON(ret);
	}
...
static int psb_handle_copyback(struct drm_device *dev,
			       struct psb_context *context, int ret)
{
	int err = ret;
	struct ttm_validate_buffer *entry;
		     ...
		     struct drm_file *file_priv)
{
	...
	ret = psb_reference_buffers(file_priv, arg->buffer_list, context);
	...
	engine = arg->engine;
	ret = psb_validate_buffer_list(file_priv, engine, context, &po_correct);
	if (unlikely(ret != 0))
		goto out_err3;
