...
const u16 reg_addr_offset[] = {V2P85SCNT_ADDR, V2P85SXCNT_ADDR};
const u8 initial_state[] = {ON, OFF};
...
static int intel_pmic_reg_initialize(u16 pmic_reg, u8 state)
{
	...
	if (state)
		val = (u8)(reg_value | REG_CNT_ENBL | REG_ON);
	else
		val = (u8)((reg_value & REG_OFF) | REG_CNT_ENBL);
...
static int intel_pmic_reg_enable(struct regulator_dev *rdev)
{
	...
	return intel_mid_pmic_writeb(pmic_info->pmic_reg,
				(u8)(reg_value | REG_ON | REG_CNT_ENBL));
}
/**
...
static int intel_pmic_reg_disable(struct regulator_dev *rdev)
{
	...
	return intel_mid_pmic_writeb(pmic_info->pmic_reg,
				(u8)((reg_value | REG_CNT_ENBL) & REG_OFF));
}
/**
...
static int __devinit crystal_cove_pmic_probe(struct platform_device *pdev)
{
...
static int __devexit crystal_cove_pmic_remove(struct platform_device *pdev)
{
...
static struct platform_driver crystal_cove_pmic_driver = {
	...
	.remove = __devexit_p(crystal_cove_pmic_remove),
