...
//Shouchung add, for Settings' touch information
 ...
 static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    {
        GTP_ERROR("I2C communication ERROR!");
        return ret; //Shouchung add, for touch can work normally
    }
...
//Shouchung add, for sleeping beauty
...
void sb_tp_0(void)
{
    ...
    printk(KERN_INFO "[SleepingBeauty] Try to send i2c message\n");
    ts = i2c_get_clientdata(i2c_connect_client);
    ret = gtp_i2c_read_no_rst(ts->client, buf, sizeof(buf));
    if (ret < 0) {
        //data->current_value = 0;
        printk(KERN_INFO "[SleepingBeauty] Result: Send i2c message fail, may enter sleep\n");
    } else {
        //data->current_value = 0xFFFF;
        printk(KERN_ERR "[SleepingBeauty] Result: Send i2c message pass, not sleep\n");
    }
}
EXPORT_SYMBOL(sb_tp_0);
