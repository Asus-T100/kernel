...
static void __dump_stream_pipe_config(struct atomisp_sub_device *isp_subdev)
{
	...
	for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++) {
		if (!isp_subdev->stream_env.pipes[i])
			continue;
		p_config = &isp_subdev->stream_env.pipe_configs[i];
		pe_config = &isp_subdev->stream_env.pipe_extra_configs[i];
	...
	s_config = &isp_subdev->stream_env.stream_config;
...
static int __destroy_stream(struct atomisp_sub_device *isp_subdev, bool force)
{
	struct atomisp_device *isp = isp_subdev->isp;
	int i;

	if (!isp_subdev->stream_env.stream)
		return -EINVAL;

	if (!force) {
		for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++)
			if (isp_subdev->stream_env.update_pipe[i])
				break;
	...
	if (isp_subdev->stream_env.stream_state == CSS_STREAM_STARTED
	    && ia_css_stream_stop(isp_subdev->stream_env.stream) !=
	    IA_CSS_SUCCESS) {
		dev_err(isp->dev, "stop stream failed.\n");
		return -EINVAL;
	}

	if (ia_css_stream_destroy(isp_subdev->stream_env.stream) !=
	    IA_CSS_SUCCESS) {
		dev_err(isp->dev, "destroy stream failed.\n");
		return -EINVAL;
	}
	isp_subdev->stream_env.stream = NULL;

	return 0;
}

static int __create_stream(struct atomisp_sub_device *isp_subdev)
{
	int pipe_index = 0, i;
	...
	for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++) {
		if (isp_subdev->stream_env.pipes[i])
			multi_pipes[pipe_index++] =
				isp_subdev->stream_env.pipes[i];
	}

	if (ia_css_stream_create(&isp_subdev->stream_env.stream_config,
	    pipe_index, multi_pipes, &isp_subdev->stream_env.stream)
	    != IA_CSS_SUCCESS)
		return -EINVAL;
...
static int __destroy_pipes(struct atomisp_sub_device *isp_subdev, bool force)
{
	struct atomisp_device *isp = isp_subdev->isp;
	int i;
	int ret = 0;
	...
	for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++) {
		if (!isp_subdev->stream_env.pipes[i]
				|| !(force
				     || isp_subdev->stream_env.update_pipe[i]))
			continue;

		if (ia_css_pipe_destroy(isp_subdev->stream_env.pipes[i])
		    != IA_CSS_SUCCESS) {
			dev_err(isp->dev,
		...
		isp_subdev->stream_env.pipes[i] = NULL;
		isp_subdev->stream_env.update_pipe[i] = false;
	}

	return ret;
}

static int __create_pipe(struct atomisp_sub_device *isp_subdev)
{
	...
	for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++) {
		if (!isp_subdev->stream_env.pipe_configs[i].
			output_info.res.width)
			continue;

		if (!memcmp(
			    &extra_config,
			    &isp_subdev->stream_env.pipe_extra_configs[i],
			    sizeof(extra_config)))
			ret = ia_css_pipe_create(
					&isp_subdev->stream_env.pipe_configs[i],
					&isp_subdev->stream_env.pipes[i]);
		else
		...
		dev_dbg(isp->dev,
			...
			"vf_h=%d vf_f=%d.\n", i,
		isp_subdev->stream_env.pipe_configs[i].output_info.res.width,
		isp_subdev->stream_env.pipe_configs[i].output_info.res.height,
		isp_subdev->stream_env.pipe_configs[i].output_info.format,
		isp_subdev->stream_env.pipe_configs[i].vf_output_info.res.width,
		isp_subdev->stream_env.pipe_configs[i].vf_output_info.res.
		height,
		isp_subdev->stream_env.pipe_configs[i].vf_output_info.format);
	}
...
pipe_err:
	for (i--; i >= 0; i--) {
		if (isp_subdev->stream_env.pipes[i]) {
			ia_css_pipe_destroy(isp_subdev->stream_env.pipes[i]);
			isp_subdev->stream_env.pipes[i] = NULL;
		}
	}
...
void atomisp_css_uninit(struct atomisp_device *isp)
{
	...
	atomisp_isp_parameters_clean_up(&isp_subdev->params.config);
...
void atomisp_css_init_struct(struct atomisp_sub_device *isp_subdev)
{
	int i;
	...
	ia_css_stream_config_defaults(&isp_subdev->stream_env.stream_config);
}

int atomisp_q_video_buffer_to_css(struct atomisp_sub_device *isp_subdev,
			...
			enum atomisp_css_pipe_id css_pipe_id)
{
	...
	err = ia_css_pipe_enqueue_buffer(
				isp_subdev->stream_env.pipes[css_pipe_id],
				&css_buf);
	if (err != IA_CSS_SUCCESS)
		return -EINVAL;
...
int atomisp_q_s3a_buffer_to_css(struct atomisp_sub_device *isp_subdev,
...
int atomisp_q_dis_buffer_to_css(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_start(struct atomisp_sub_device *isp_subdev,
			enum atomisp_css_pipe_id pipe_id, bool in_reset)
{
	struct atomisp_device *isp = isp_subdev->isp;
	int ret = 0;
	if (in_reset) {
		if (__destroy_stream(isp_subdev, true))
			dev_warn(isp->dev, "destroy stream failed.\n");

		if (__destroy_pipes(isp_subdev, true))
	...
	if (ia_css_stream_start(isp_subdev->stream_env.stream) !=
	    IA_CSS_SUCCESS) {
	...
	isp_subdev->stream_env.stream_state = CSS_STREAM_CREATED;
	return 0;
...
{
	ia_css_stream_set_isp_config(isp_subdev->stream_env.stream,
				     &isp_subdev->params.config);
	atomisp_isp_parameters_clean_up(&isp_subdev->params.config);
}

int atomisp_css_queue_buffer(struct atomisp_sub_device *isp_subdev,
			     enum atomisp_css_pipe_id pipe_id,
			     enum atomisp_css_buffer_type buf_type,
			     struct atomisp_css_buffer *isp_css_buffer)
{
	if (ia_css_pipe_enqueue_buffer(isp_subdev->stream_env.pipes[pipe_id],
...
int atomisp_css_dequeue_buffer(struct atomisp_sub_device *isp_subdev,
				enum atomisp_css_pipe_id pipe_id,
				enum atomisp_css_buffer_type buf_type,
				struct atomisp_css_buffer *isp_css_buffer)
{
	...
	err = ia_css_pipe_dequeue_buffer(isp_subdev->stream_env.pipes[pipe_id],
					&isp_css_buffer->css_buffer);
	if (err != IA_CSS_SUCCESS) {
		dev_err(isp->dev,
...
int atomisp_css_allocate_3a_dis_bufs(struct atomisp_sub_device *isp_subdev,
				struct atomisp_s3a_buf *s3a_buf,
				struct atomisp_dis_buf *dis_buf)
{
	struct atomisp_device *isp = isp_subdev->isp;
...
void atomisp_css_free_3a_dis_buffers(struct atomisp_sub_device *isp_subdev)
{
	...
	if (isp_subdev->params.curr_grid_info.dvs_grid.enable) {
		...
		isp_subdev->params.dis_proj_data_valid = false;
...
int atomisp_css_get_grid_info(struct atomisp_sub_device *isp_subdev,
				enum atomisp_css_pipe_id pipe_id)
{
	...
	ia_css_pipe_get_info(isp_subdev->stream_env.pipes[pipe_id], &p_info);
	...
	if ((!memcmp(&old_info, &isp_subdev->params.curr_grid_info,
		...
		dev_dbg(isp->dev,
			...
			"dvs_stat ver coef addr=0x%x\n",
			 !memcmp(&old_info,
				 &isp_subdev->params.curr_grid_info,
				 sizeof(old_info)),
			 ...
			 (unsigned int)isp_subdev->params.dvs_coeff->hor_coefs,
			 (unsigned int)isp_subdev->params.dvs_coeff->ver_coefs);
		return -EINVAL;
	}
...
int atomisp_alloc_3a_output_buf(struct atomisp_sub_device *isp_subdev)
{
	isp_subdev->params.s3a_user_stat = ia_css_3a_statistics_allocate(
				&isp_subdev->params.curr_grid_info.s3a_grid);
...
int atomisp_alloc_dis_coef_buf(struct atomisp_sub_device *isp_subdev)
{
	if (!isp_subdev->params.curr_grid_info.dvs_grid.enable)
		return 0;

	/* DIS coefficients. */
	isp_subdev->params.dvs_coeff = ia_css_dvs_coefficients_allocate(
				&isp_subdev->params.curr_grid_info.dvs_grid);
	...
	/* DIS projections. */
	isp_subdev->params.dis_proj_data_valid = false;
	isp_subdev->params.dvs_stat = ia_css_dvs_statistics_allocate(
				&isp_subdev->params.curr_grid_info.dvs_grid);
...
int atomisp_css_get_3a_statistics(struct atomisp_sub_device *isp_subdev,
				  struct atomisp_css_buffer *isp_css_buffer)
{
	if (isp_subdev->params.s3a_user_stat &&
			isp_subdev->params.s3a_output_bytes) {
...
void atomisp_css_get_dis_statistics(struct atomisp_sub_device *isp_subdev,
				    struct atomisp_css_buffer *isp_css_buffer)
{
	if (isp_subdev->params.dvs_stat) {
		ia_css_get_dvs_statistics(isp_subdev->params.dvs_stat,
				  isp_css_buffer->css_buffer.data.stats_dvs);
		isp_subdev->params.dis_proj_data_valid = true;
	}
}
...
int atomisp_css_input_set_resolution(struct atomisp_sub_device *isp_subdev,
					struct v4l2_mbus_framefmt *ffmt)
{
	isp_subdev->stream_env.stream_config.input_res.width = ffmt->width;
	isp_subdev->stream_env.stream_config.input_res.height = ffmt->height;
	return 0;
}

void atomisp_css_input_set_binning_factor(struct atomisp_sub_device *isp_subdev,
						unsigned int bin_factor)
{
	isp_subdev->stream_env.stream_config.sensor_binning_factor = bin_factor;
}

void atomisp_css_input_set_bayer_order(struct atomisp_sub_device *isp_subdev,
				enum atomisp_css_bayer_order bayer_order)
{
	isp_subdev->stream_env.stream_config.bayer_order = bayer_order;
}

void atomisp_css_input_set_format(struct atomisp_sub_device *isp_subdev,
					enum atomisp_css_stream_format format)
{
	isp_subdev->stream_env.stream_config.format = format;
}
...
{
	isp_subdev->stream_env.stream_config.effective_res.width = width;
	isp_subdev->stream_env.stream_config.effective_res.height = height;

	return 0;
}

void atomisp_css_video_set_dis_envelope(struct atomisp_sub_device *isp_subdev,
					unsigned int dvs_w, unsigned int dvs_h)
{
	isp_subdev->stream_env.pipe_configs[0].dvs_envelope.width = dvs_w;
	isp_subdev->stream_env.pipe_configs[0].dvs_envelope.height = dvs_h;
}
					...
					bool two_ppc)
{
	...
	if (isp_subdev->stream_env.stream_config.two_pixels_per_clock ==
	    !!two_ppc)
		return;

	isp_subdev->stream_env.stream_config.two_pixels_per_clock = !!two_ppc;
	for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++)
		isp_subdev->stream_env.update_pipe[i] = true;
}

void atomisp_css_enable_raw_binning(struct atomisp_sub_device *isp_subdev,
					bool enable)
{
	int i;

	for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++) {
		if (isp_subdev->stream_env.pipe_extra_configs[i].
		    enable_raw_binning == !!enable)
			continue;

		isp_subdev->stream_env.pipe_extra_configs[i].enable_raw_binning
							= !!enable;
		isp_subdev->stream_env.update_pipe[i] = true;
	}
}

void atomisp_css_enable_dz(struct atomisp_sub_device *isp_subdev, bool enable)
{
	int i;
	for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++)
		isp_subdev->stream_env.pipe_extra_configs[i].enable_dz = enable;
}

void atomisp_css_capture_set_mode(struct atomisp_sub_device *isp_subdev,
				enum atomisp_css_capture_mode mode)
{
	if (isp_subdev->stream_env.pipe_configs[IA_CSS_PIPE_ID_CAPTURE].
					default_capture_config.mode == mode)
		return;

	isp_subdev->stream_env.pipe_configs[IA_CSS_PIPE_ID_CAPTURE].
					default_capture_config.mode = mode;
	isp_subdev->stream_env.update_pipe[IA_CSS_PIPE_ID_CAPTURE] = true;
}

void atomisp_css_input_set_mode(struct atomisp_sub_device *isp_subdev,
				enum atomisp_css_input_mode mode)
{
	isp_subdev->stream_env.stream_config.mode = mode;
...
void atomisp_css_capture_enable_online(struct atomisp_sub_device *isp_subdev,
							bool enable)
{
	if (isp_subdev->stream_env.stream_config.online == !!enable)
		return;

	isp_subdev->stream_env.stream_config.online = !!enable;
	isp_subdev->stream_env.update_pipe[IA_CSS_PIPE_ID_CAPTURE] = true;
}

void atomisp_css_preview_enable_online(struct atomisp_sub_device *isp_subdev,
							bool enable)
{
	int i;
...
void atomisp_css_enable_continuous(struct atomisp_sub_device *isp_subdev,
							bool enable)
{
	int i;

	if (isp_subdev->stream_env.stream_config.continuous != !!enable) {
		isp_subdev->stream_env.stream_config.continuous = !!enable;
		for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++)
			isp_subdev->stream_env.update_pipe[i] = true;
	}
}
...
int atomisp_css_input_configure_port(struct atomisp_sub_device *isp_subdev,
					mipi_port_ID_t port,
					unsigned int num_lanes,
					unsigned int timeout)
{
	isp_subdev->stream_env.stream_config.source.port.port = port;
	isp_subdev->stream_env.stream_config.source.port.num_lanes = num_lanes;
	isp_subdev->stream_env.stream_config.source.port.timeout = timeout;
...
int atomisp_css_set_black_frame(struct atomisp_sub_device *isp_subdev,
				const struct atomisp_css_frame *raw_black_frame)
{
	if (sh_css_set_black_frame(isp_subdev->stream_env.stream,
				   raw_black_frame)
	    != IA_CSS_SUCCESS)
		return -ENOMEM;
...
int atomisp_css_stop(struct atomisp_sub_device *isp_subdev,
			enum atomisp_css_pipe_id pipe_id, bool in_reset)
{
	struct atomisp_device *isp = isp_subdev->isp;
	...
	isp_subdev->stream_env.stream_state = CSS_STREAM_STOPPED;
	if (!in_reset) {
		int i;
		for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++) {
			ia_css_pipe_config_defaults(
		...
		ia_css_stream_config_defaults(
					&isp_subdev->stream_env.stream_config);
	}
...
int atomisp_css_continuous_set_num_raw_frames(
					...
					int num_frames)
{
	if (ia_css_stream_set_buffer_depth(isp_subdev->stream_env.stream,
					   num_frames)
	    != IA_CSS_SUCCESS)
		return -EINVAL;
...
void atomisp_css_disable_vf_pp(struct atomisp_sub_device *isp_subdev,
			       bool disable)
{
	int i;

	for (i = 0; i < IA_CSS_PIPE_ID_NUM; i++)
		isp_subdev->stream_env.pipe_extra_configs[i].disable_vf_pp =
			!!disable;
}
			       ...
			       enum ia_css_pipe_id pipe_id)
{
	...
	isp_subdev->stream_env.pipe_configs[pipe_id].mode =
				__pipe_id_to_pipe_mode(pipe_id);
	isp_subdev->stream_env.update_pipe[pipe_id] = true;

	isp_subdev->stream_env.pipe_configs[pipe_id].output_info.res.width =
		width;
	isp_subdev->stream_env.pipe_configs[pipe_id].output_info.res.height =
		height;
	isp_subdev->stream_env.pipe_configs[pipe_id].output_info.format =
		format;
				 ...
				 enum ia_css_pipe_id pipe_id)
{
	...
	isp_subdev->stream_env.pipe_extra_configs[pipe_id].enable_yuv_ds =
		true;
	isp_subdev->stream_env.pipe_configs[pipe_id].bayer_ds_out_res.width =
		width;
	isp_subdev->stream_env.pipe_configs[pipe_id].bayer_ds_out_res.height =
		height;
				  ...
				  enum ia_css_pipe_id pipe_id)
{
	...
	isp_subdev->stream_env.pipe_configs[pipe_id].vf_output_info.res.width =
		width;
	isp_subdev->stream_env.pipe_configs[pipe_id].vf_output_info.res.height =
		height;
	isp_subdev->stream_env.pipe_configs[pipe_id].vf_output_info.format =
		format;
	dev_dbg(isp->dev,
...
static int __get_frame_info(struct atomisp_sub_device *isp_subdev,
				...
				enum ia_css_pipe_id pipe_id)
{
	...
	ret = ia_css_pipe_get_info(isp_subdev->stream_env.pipes[pipe_id],
				   &p_info);
	if (ret == IA_CSS_SUCCESS) {
		switch (type) {
					...
					uint16_t source_pad)
{
	...
	case ATOMISP_SUBDEV_PAD_SOURCE_CAPTURE:
		if (isp_subdev->run_mode->val == ATOMISP_RUN_MODE_VIDEO
		    || isp_subdev->vfpp->val == ATOMISP_VFPP_DISABLE_SCALER)
...
int atomisp_get_css_frame_info(struct atomisp_sub_device *isp_subdev,
				uint16_t source_pad,
				struct atomisp_css_frame_info *frame_info)
{
	...
	ia_css_pipe_get_info(isp_subdev->stream_env.pipes[pipe_index], &info);
	switch (source_pad) {
	case ATOMISP_SUBDEV_PAD_SOURCE_CAPTURE:
...
int atomisp_css_preview_configure_output(struct atomisp_sub_device *isp_subdev,
				unsigned int width, unsigned int height,
				enum atomisp_css_frame_format format)
{
...
int atomisp_css_capture_configure_output(struct atomisp_sub_device *isp_subdev,
				unsigned int width, unsigned int height,
				enum atomisp_css_frame_format format)
{
	__configure_output(isp_subdev, width, height, format,
...
int atomisp_css_video_configure_output(struct atomisp_sub_device *isp_subdev,
				unsigned int width, unsigned int height,
				enum atomisp_css_frame_format format)
{
	__configure_output(isp_subdev, width, height, format,
...
int atomisp_css_video_get_viewfinder_frame_info(
					...
					struct atomisp_css_frame_info *info)
{
	return __get_frame_info(isp_subdev, info, ATOMISP_CSS_VF_FRAME,
						IA_CSS_PIPE_ID_VIDEO);
}
...
int atomisp_css_capture_get_output_raw_frame_info(
					...
					struct atomisp_css_frame_info *info)
{
	return __get_frame_info(isp_subdev, info, ATOMISP_CSS_RAW_FRAME,
						IA_CSS_PIPE_ID_CAPTURE);
}
...
int atomisp_css_preview_get_output_frame_info(
					...
					struct atomisp_css_frame_info *info)
{
	return __get_frame_info(isp_subdev, info, ATOMISP_CSS_OUTPUT_FRAME,
						IA_CSS_PIPE_ID_PREVIEW);
}
...
int atomisp_css_offline_capture_configure(struct atomisp_sub_device *isp_subdev,
			int num_captures, unsigned int skip, int offset)
{
	enum ia_css_err ret;

	ret = ia_css_stream_capture(isp_subdev->stream_env.stream,
...
int atomisp_css_capture_enable_xnr(struct atomisp_sub_device *isp_subdev,
				   bool enable)
{
	...
	ia_css_stream_get_isp_config(isp_subdev->stream_env.stream,
				     &isp_config);

	if (isp_subdev->params.capture_config.enable_xnr != enable)
		isp_subdev->params.capture_config.enable_xnr = enable;

	return 0;
}

void atomisp_css_send_input_frame(struct atomisp_sub_device *isp_subdev,
				  unsigned short *data, unsigned int width,
				  unsigned int height)
{
	ia_css_stream_send_input_frame(isp_subdev->stream_env.stream,
					data, width, height);
}
...
void atomisp_css_request_flash(struct atomisp_sub_device *isp_subdev)
{
	ia_css_stream_request_flash(isp_subdev->stream_env.stream);
}

void atomisp_css_set_wb_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_wb_config *wb_config)
{
	isp_subdev->params.config.wb_config = wb_config;
}

void atomisp_css_set_ob_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_ob_config *ob_config)
{
	isp_subdev->params.config.ob_config = ob_config;
}

void atomisp_css_set_dp_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_dp_config *dp_config)
{
	isp_subdev->params.config.dp_config = dp_config;
}

void atomisp_css_set_de_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_de_config *de_config)
{
	isp_subdev->params.config.de_config = de_config;
}

void atomisp_css_set_default_de_config(struct atomisp_sub_device *isp_subdev)
{
	isp_subdev->params.config.de_config = NULL;
}

void atomisp_css_set_ce_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_ce_config *ce_config)
{
	isp_subdev->params.config.ce_config = ce_config;
}

void atomisp_css_set_nr_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_nr_config *nr_config)
{
	isp_subdev->params.config.nr_config = nr_config;
}

void atomisp_css_set_ee_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_ee_config *ee_config)
{
	isp_subdev->params.config.ee_config = ee_config;
}

void atomisp_css_set_tnr_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_tnr_config *tnr_config)
{
	isp_subdev->params.config.tnr_config = tnr_config;
}

void atomisp_css_set_cc_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_cc_config *cc_config)
{
	isp_subdev->params.config.cc_config = cc_config;
}

void atomisp_css_set_macc_table(struct atomisp_sub_device *isp_subdev,
...
{
	isp_subdev->params.config.yuv2rgb_cc_config = yuv2rgb_cc_config;
}
...
{
	isp_subdev->params.config.anr_thres = anr_thres;
}

void atomisp_css_set_gc_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_gc_config *gc_config)
{
	isp_subdev->params.config.gc_config = gc_config;
}

void atomisp_css_set_3a_config(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_3a_config *s3a_config)
{
	isp_subdev->params.config.s3a_config = s3a_config;
}

void atomisp_css_video_set_dis_vector(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_set_dis_coefs(struct atomisp_sub_device *isp_subdev,
			  struct atomisp_dis_coefficients *coefs)
{
	...
	if (copy_from_user(isp_subdev->params.dvs_coeff->hor_coefs,
	    coefs->horizontal_coefficients,
	    isp_subdev->params.dvs_hor_coef_bytes))
		return -EFAULT;
	if (copy_from_user(isp_subdev->params.dvs_coeff->ver_coefs,
...
void atomisp_css_set_zoom_factor(struct atomisp_sub_device *isp_subdev,
					unsigned int zoom)
{
	struct atomisp_device *isp = isp_subdev->isp;
...
int atomisp_css_get_wb_config(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_ob_config(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_dp_config(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_de_config(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_nr_config(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_ee_config(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_tnr_config(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_ctc_table(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_gamma_table(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_gc_config(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_3a_config(struct atomisp_sub_device *isp_subdev,
...
int atomisp_css_get_zoom_factor(struct atomisp_sub_device *isp_subdev,
					unsigned int *zoom)
{
...
void atomisp_css_set_shading_table(struct atomisp_sub_device *isp_subdev,
			struct atomisp_css_shading_table *table)
{
	isp_subdev->params.config.shading_table = table;
}
...
void atomisp_css_set_morph_table(struct atomisp_sub_device *isp_subdev,
					struct atomisp_css_morph_table *table)
{
	isp_subdev->params.config.morph_table = table;
}

void atomisp_css_get_morph_table(struct atomisp_sub_device *isp_subdev,
				struct atomisp_css_morph_table *table)
{
	struct ia_css_isp_config isp_config;
	struct atomisp_device *isp = isp_subdev->isp;
