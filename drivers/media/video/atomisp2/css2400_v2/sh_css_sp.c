 ...
 */
	...
	sh_css_dtrace(SH_DBG_TRACE, "load binary: %s\n", binary_name);
			...
			const mipi_port_ID_t port_id)
{
	...
	/* Init pipeline data */
		sh_css_sp_init_group(two_ppc, first_binary->input_format, offline,(uint8_t) (port_id - MIPI_PORT0_ID));
	...
	for (stage = me->stages, num = 0; stage; stage = stage->next, num++) {
		...
		if (is_sp_stage(stage)) {
			...
			sp_init_stage(stage, pipe_num, num, preview_mode,
				low_light, xnr, (int) (sh_css_sp_group.pipe[thread_id].port_id - MIPI_PORT0_ID));
		}
...
sh_css_event_init_irq_mask(void)
{
	...
	event_irq_mask_init.and_mask = IA_CSS_EVENT_TYPE_ALL;
...
//{
//	enum ia_css_pipe_id pipe_id;
//	ia_css_temp_pipe_to_pipe_id(pipe, &pipe_id);
//	return ia_css_pipe_id_set_irq_mask(pipe_id, or_mask, and_mask);
//}

enum ia_css_err
ia_css_pipe_id_set_irq_mask(
	enum ia_css_pipe_id pipe_id,
	unsigned int or_mask,
	unsigned int and_mask)
{
