...
#define DVS_TABLE_IN_BLOCKDIM_X(X)   		(((X)/ DVS_BLOCKDIM_X) + 1)
#define DVS_TABLE_IN_BLOCKDIM_Y_LUMA(X)		(((X)/ DVS_BLOCKDIM_Y_LUMA) + 1)
#define DVS_TABLE_IN_BLOCKDIM_Y_CHROMA(X)	(((X)/ DVS_BLOCKDIM_Y_CHROMA) + 1)
...
generate_dvs_6axis_table(const struct ia_css_resolution	*frame_res, const struct ia_css_resolution *dvs_offset)
{
	...
	{	/*Initialize new struct with latest config settings*/	
		...
		dvs_config->width_uv = width_uv = DVS_TABLE_IN_BLOCKDIM_X(frame_res->width / 2); /* = Y/2, depens on colour format YUV 4.2.0*/
		dvs_config->height_uv = height_uv = DVS_TABLE_IN_BLOCKDIM_Y_CHROMA(frame_res->height / 2);   				

		sh_css_dtrace(SH_DBG_TRACE, "generate_dvs_6axis_table W %d H %d\n",width_y,height_y);
		...
		{
			sh_css_dtrace(SH_DBG_TRACE, "generate_dvs_6axis_table: %d\n, leave() ",(int)err);	
			dvs_config = NULL;
		}
		else
		{
...
free_dvs_6axis_table(struct ia_css_dvs_6axis_config  **dvs_6axis_config)
{
	...
	{	
		sh_css_dtrace(SH_DBG_TRACE, "free_dvs_6axis_table dvs_6axis_config->xcoords_y %p\n",(*dvs_6axis_config)->xcoords_y);
		if((*dvs_6axis_config)->xcoords_y)
		{
			 sh_css_free((*dvs_6axis_config)->xcoords_y);
			 (*dvs_6axis_config)->xcoords_y = NULL;
		}
		sh_css_dtrace(SH_DBG_TRACE, "free_dvs_6axis_table dvs_6axis_config->ycoords_y %p\n",(*dvs_6axis_config)->ycoords_y);
...
/* TODO : allcoate UV buffers in generate funct()		
