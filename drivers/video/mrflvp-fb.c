...
static void init_display_controller_registers(void)
{
	...
	int width = MRFL_FB_HACT;
	int height = MRFL_FB_VACT;
	int viewport_width = MRFL_FB_HACT;
	int viewport_height = MRFL_FB_VACT;
	...
	iowrite32((MRFL_FB_RGB888 << 7) | 0x12, io_virt + 0xB00c);
	...
	/* Programming Pipe A */
	iowrite32((((MRFL_FB_HACT + 80 - 1) << 16) | (MRFL_FB_HACT - 1)),
		  io_virt + 0x60000);
	iowrite32((((MRFL_FB_HACT + 80 - 1) << 16) | (MRFL_FB_HACT - 1)),
		  io_virt + 0x60004);
	iowrite32(((MRFL_FB_HACT + 48 - 1) << 16) | (MRFL_FB_HACT + 8 - 1),
		  io_virt + 0x60008);
	iowrite32((((MRFL_FB_VACT + 5) << 16) | (MRFL_FB_VACT - 1)),
		  io_virt + 0x6000C);
	iowrite32((((MRFL_FB_VACT + 5) << 16) | (MRFL_FB_VACT - 1)),
		  io_virt + 0x60010);
	iowrite32((((MRFL_FB_VACT + 2) << 16) | (MRFL_FB_VACT + 1)),
		  io_virt + 0x60014);
	iowrite32((((MRFL_FB_HACT - 1) << 16) | (MRFL_FB_VACT - 1)),
		  io_virt + 0x6001C);
			       ...
			       const struct pci_device_id *id)
{
	...
	pci_read_config_dword(pdev, MRFL_FB_PCI_FB_BASE, &fb_base);
	...
	merrifield_fb_fix.smem_len = MRFL_FB_HACT * MRFL_FB_VACT * 4;
	...
	ret = fb_alloc_cmap(&info->cmap, MRFL_FB_CMAP_ALLOC_LEN, 0);
	if (ret < 0) {
	...
	info->cmap.len = MRFL_FB_CMAP_LEN;
	info->fix.type = FB_TYPE_PACKED_PIXELS;
	info->fix.visual = FB_VISUAL_TRUECOLOR;
	info->fix.line_length = width * 2;
	info->fix.accel = FB_ACCEL_NONE;
	info->fix.ypanstep = MRFL_FB_YPAN_STEP;
	...
	info->var.bits_per_pixel = MRFL_FB_BPP;
	info->var.height = height;
	info->var.width = width;
	info->var.red.offset = MRFL_FB_RED_OFS;
	info->var.red.length = MRFL_FB_RED_LEN;
	info->var.green.offset = MRFL_FB_GREEN_OFS;
	info->var.green.length = MRFL_FB_GREEN_LEN;
	info->var.blue.offset = MRFL_FB_BLUE_OFS;
	info->var.blue.length = MRFL_FB_BLUE_LEN;
	...
	info->pixmap.addr = kmalloc(MRFL_FB_PIXMAP_SIZE, GFP_KERNEL);
	if (!info->pixmap.addr) {
		dev_err(&pdev->dev, "pixmap allocation failure\n");
		goto failure;
	}

	info->pixmap.size = MRFL_FB_PIXMAP_SIZE;
	info->pixmap.buf_align = MRFL_FB_PIXMAP_BALIGN;
	info->pixmap.scan_align = MRFL_FB_PIXMAP_SALIGN;
	info->pixmap.access_align = MRFL_FB_PIXMAP_AALIGN;
