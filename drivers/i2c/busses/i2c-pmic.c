...
static int __devinit pmic_i2c_probe(struct ipc_device *ipcdev)
{
	...
	if (ret) {
		dev_err(&ipcdev->dev, "Error adding the adapter\n");
		goto err_adap_add;
	}
...
static int pmic_i2c_remove(struct ipc_device *ipcdev)
{
	iounmap(pmic_dev->pmic_intr_map);
	free_irq(pmic_dev->irq, pmic_dev);
...
struct ipc_driver pmic_i2c_driver = {
...
{
	return ipc_driver_register(&pmic_i2c_driver);
}

static void __exit pmic_i2c_exit(void)
{
	ipc_driver_unregister(&pmic_i2c_driver);
}
