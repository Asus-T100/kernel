...
#define NFC_INT  53
#define NFC_WAKE  51
#define NFC_ENABLE 59
#define BCM_NFC_BUS 3
#define NFC_ACPI
#endif
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (!gpio_is_valid(platform_data->wake_gpio)) {
		pr_err("%s: gpio %s not found\n", __func__, "NFC_WAKE");
		return -ENODEV;
	}
...
static struct acpi_device_id nfc_acpi_match[] = {
	{"BCM2079", 0},
	{}
};

MODULE_DEVICE_TABLE(acpi, nfc_acpi_match);
#endif

#ifndef NFC_ACPI
static int __init nfc_attach(struct i2c_adapter *adapter)
{
	struct i2c_client *i2c_client;

	if (adapter->nr != BCM_NFC_BUS)
		return 0;

	i2c_client = i2c_new_device(adapter, &bcm2079x_i2c_device);
	...
	*/
	list_add_tail(&i2c_client->detected, &i2c_client->driver->clients);
	return 0;
}
...
static struct i2c_driver bcm2079x_driver = {
	...
	.driver = {
		...
		.acpi_match_table = ACPI_PTR(nfc_acpi_match),

#endif
	},
#ifndef NFC_ACPI

	.attach_adapter = nfc_attach,
...
static int __init bcm2079x_dev_init(void)
{
	...
	int sku_id = asustek_boardinfo_get(FUN_SKU_ID);

	if (project_id == 5 && sku_id == 0)
		return 0;
	else
