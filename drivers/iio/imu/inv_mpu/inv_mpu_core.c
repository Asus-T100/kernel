...
static ssize_t get_mpu65xx_compass_rawdata(struct device *dev,
		...
		const struct i2c_device_id *id)
{
	...
	case INV_MPU6500:
		if (SECONDARY_SLAVE_TYPE_COMPASS == st->plat_data.sec_slave_type) {
			...
			st->num_channels = INV_CHANNEL_NUM_GYRO_ACCL_QUANTERNION;
		}
		break;
	case INV_MPU3050:
		...
		const struct i2c_device_id *id)
{
	...
	// GPIO_S0_59
	st->plat_data.int_gpio = acpi_get_gpio_by_index(&client->dev, 0, NULL);
	...
	printk("inv_mpu probe client irq[%d] to gpio[%d]\n",
			st->client->irq, st->plat_data.int_gpio);
#endif
	...
	if (!strcmp(id->name, "MPU6500:00")||!strcmp(id->name, "INVN6500:00")) {
		// notice it's lower case letter for ml_sys_helper.c name matching
		strcpy(indio_dev->name,"mpu6500");
...
static int __init inv_mpu_init(void)
{
	...
	if (result) {
		printk("mpu6500: i2c_add_driver failed\n");
		return result;
	}
	return 0;
}
