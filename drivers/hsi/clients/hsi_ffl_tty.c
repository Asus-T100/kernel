			    ...
			    const char *name, const char *info)
{
	...
	if (err) {
		pr_err(DRVNAME ": gpio_request(gpio, label) failed (%d, %s)",
				gpio, label);
	...
	if (err) {
		pr_err(DRVNAME ": gpio_direction failed (%d, %s, %d)",
				gpio, label, dir);
		goto exit_error;
	}
	...
	if (err) {
		pr_err(DRVNAME ": gpio_export failed (%d, %s, %d, err:%d)",
				gpio, label, dir_may_change, err);
		goto exit_error;
	}
...
exit_error:
	gpio_free(gpio);
no_gpio_request:
	pr_err(DRVNAME ": unable to configure GPIO%d (%s), err:%d", gpio, info, err);
	return err;
}
