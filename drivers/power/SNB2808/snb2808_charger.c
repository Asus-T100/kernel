...
#define DRV_NAME "snb2808_charger"
#define DEV_NAME "SNB2808"
#include <linux/acpi_gpio.h>
#include <linux/mfd/intel_mid_pmic.h>
#include <linux/regulator/of_regulator.h>
#include <linux/regulator/consumer.h>

#include <snb2808_charger.h>
...
#define CHARGER_PS_NAME				"snb2808_charger"
...
static int program_timers(struct bq24192_chip *chip, int wdt_duration,
				bool sfttmr_enable);
/*SDP 32 1d 54 11 d7 b4 73, 4b 6c 90 60*/
...
static void otg_worker(struct work_struct *work)
{
	...
	if((gpio_data == 0)&&(OtgOk == 0)){
		...
		OtgOk = 1;
              cur_cable_status = UG31XX_NO_CABLE;
              cable_status = UG31XX_NO_CABLE;
		if(chip_extern->regulator_v3p3s != NULL && !v3p3_use){
			v3p3_use++;
...
void asus_charge_enable(int enable)
{
	if(enable){
		printk("Start charging\n");
		bq24192_reg_read_modify(chip_extern->client,BQ24192_POWER_ON_CFG_REG,0x10, true);
...
static int TempArry[5][2] = {{1000,500},{550,450},{500,100},{150,0},{50,-200}};
static void JetiaWork(struct work_struct *work)
{
	...
	switch (index)
	{
		case 0:
			...
			bq24192_set_cc(chip_extern,1520);
	...
	case POWER_SUPPLY_PROP_ENABLE_CHARGING:
		if (chip->boost_mode)
			...
			//charging = bq24192_is_charging();
                        charging = asus_charger_is_charging();
...
sched_task_work:
	...
	//if (BQ24192_CHRGR_STAT_BAT_FULL == bq24192_is_charging()) {
        if (BQ24192_CHRGR_STAT_BAT_FULL == asus_charger_is_charging()) {
		power_supply_changed(&chip->usb);
		dev_warn(&chip->client->dev,
			...
			const struct i2c_device_id *id)
{
	...
	struct bq24192_chip *chip;
	int ret,gpio_handle,gpio_data,irq;

	client->dev.platform_data = &platform_data_init;
...
#ifdef CONFIG_ACPI
static struct acpi_device_id snb2808_acpi_match[] = {
	{"SNB2808", 0},
	{}
};
MODULE_DEVICE_TABLE(acpi, snb2808_acpi_match);
#endif
...
static struct i2c_driver bq24192_i2c_driver = {
	.driver	= {
		...
		.pm	= &bq24192_pm_ops,
#ifdef CONFIG_ACPI
		.acpi_match_table = ACPI_PTR(snb2808_acpi_match),
#endif
	},
	.probe		= bq24192_probe,
