...
//max batt_capacity  --lambert
#define MAX_BATT_CAPACITY_LIMIT 60
...
struct bq24192_chip {
	...
	struct delayed_work get_v3p3sx_wrkr;
...
unsigned char VbusDetach = 1; 
unsigned char vbus_Event = 0; 
extern unsigned char PowerStateOk;
...
extern enum power_supply_charger_cable_type
	GetPowerState(void);
//extern int asus_get_batt_capacity(void);//---lambert
...
{
	int gpio_handle,gpio_data,rgrt;
...
void bq24192_charge_enable(int enable)
{
	if(enable){
		...
		//bq24192_reg_read_modify(chip_extern->client,BQ24192_INPUT_SRC_CNTL_REG,0x80, false);
		bq24192_reg_read_modify(chip_extern->client,BQ24192_POWER_ON_CFG_REG,0x10, true);
	}
	else{
		printk("Stop charging\n");
		//bq24192_reg_read_modify(chip_extern->client,BQ24192_INPUT_SRC_CNTL_REG,0x80, true);
		bq24192_reg_read_modify(chip_extern->client,BQ24192_POWER_ON_CFG_REG,0x10, false);
	}
}
...
static void JetiaWork(struct work_struct *work) 
{
	...
	switch (index)
	{
		case 0:
			...
			bq24192_set_cc(chip_extern,1856);
...
static struct proc_dir_entry *charging_limit_proc_file;
char charging_limit =0;//0 means set limit , 1 means remove limit
char limit_temp=1;
static ssize_t charging_limit_proc_write(struct file *filp, const char __user *buffer, size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	ret = copy_from_user(&limit_temp,buffer,sizeof(charging_limit));
	//0 's ASSIC is 48 , and we only receive 0 or 1 here ....@wigman
	if((limit_temp!=48)&&(limit_temp!=49))
		return -EFAULT;
	charging_limit = limit_temp-48;
	if(charging_limit){
	//if limit is removed , then enable charging
		if (chip_extern->is_charging_enabled==false){
			  ...
			  chip_extern->is_charging_enabled=true;
     		   	 int temp_type;
	 		    temp_type = GetPowerState();
	 		    bq24192_cable_callback(temp_type);
				}
		}
    return 2;
}

static struct file_operations charging_limit_proc_ops = {
    .write = charging_limit_proc_write,
};

static void create_charging_limit_proc_file(void)
{
    printk("create_charging_limit_proc_file\n");
    charging_limit_proc_file = proc_create("asus_eng_charging_limit", 0666,NULL, &charging_limit_proc_ops);
    if(!charging_limit_proc_file)
	{
		printk("create charging_limit_proc_file fail\n");
	}
}
...
static void bq24192_full_worker(struct work_struct *work)
{
	...
	po = bq24192_read_reg(chip->client, BQ24192_POWER_ON_CFG_REG);
	...
	if(OtgOk){
	 ...
	 //add by lambert
	else  if((!VbusDetach)&&(!charging_limit)){
		printk("xiaxia\n");
    		int temp_battcap=0;
		//temp_battcap=asus_get_batt_capacity();
		temp_battcap=fg_chip_get_property(POWER_SUPPLY_PROP_CAPACITY);
		printk("temp_battcap=%d\n",temp_battcap);
		if(temp_battcap>=MAX_BATT_CAPACITY_LIMIT){
			
            //if((po&0x10)==POWER_ON_CFG_CHRG_CFG_EN){
				ret=bq24192_reg_read_modify(chip_extern->client,
		                 BQ24192_POWER_ON_CFG_REG,0x10, false);
			    if(ret<0)
				printk("xia:failed to disable charging\n");
          ...
          //  }
		   ...
		   //if((po&0x10)==POWER_ON_CFG_CHRG_CFG_DIS){
			if (chip_extern->is_charging_enabled==false){
	 		  ret=bq24192_reg_read_modify(chip_extern->client,
		              BQ24192_POWER_ON_CFG_REG,0x10, true);
			  if(ret<0)
				printk("xia:failed to enable charging\n");
			  chip_extern->is_charging_enabled=true;
     		   	 int temp_type;
	 		    temp_type = GetPowerState();
	 		    bq24192_cable_callback(temp_type);
...
static void Get_v3p3sx_wrkr(struct work_struct *work)
{
    int rgrt;

	 if (chip_extern->pdata->use_regulator) {
		chip_extern->regulator_v3p3s = regulator_get(&(chip_extern->client->dev), REGULATOR_V3P3S);
		if (IS_ERR(chip_extern->regulator_v3p3s)) {
			dev_warn(&chip_extern->client->dev, "V3P3S	failed");
			printk("wigman...regulator_get V3P3S	failed\n");
			chip_extern->regulator_v3p3s = NULL;
		}else{
		rgrt = regulator_enable(chip_extern->regulator_v3p3s);
			}
	 }
	
    if(chip_extern->regulator_v3p3s != NULL){
		printk("wigman...charger get 3.3v success!");
    	}else{
    	     schedule_delayed_work(&chip_extern->get_v3p3sx_wrkr, 3*HZ);
    		}
}
				   ...
				   unsigned long event, void *param)
{
	...
	if((!OtgOk)&&vbus_Event){
		vbus_Event = 0;
		printk("wigman.....clear vbus_Event\n");
	    if(VbusDetach){
		   PowerStateOk = 0;
		   bq24192_cable_callback(POWER_SUPPLY_CHARGER_TYPE_NONE);
		}else{
			...
			const struct i2c_device_id *id)
{
	...
	create_charging_limit_proc_file();
	chip_extern = chip;
	chip->client = client;
...
static int bq24192_suspend(struct device *dev)
{
	...
	rgrt = regulator_disable(chip_extern->regulator_v3p3s);
	if(rgrt)
		printk("Failed to turn OFF 3P3SX\n");
...
{
	struct bq24192_chip *chip = dev_get_drvdata(dev);
	int ret,gpio_handle,gpio_data,rgrt;
	...
	rgrt = regulator_enable(chip_extern->regulator_v3p3s);
	if(rgrt)
		printk("Failed to turn ON 3P3SX\n");
