...
int asus_battery_info_proc_write(struct file *file, const char *buffer,
                    unsigned long count, void *data)
{
    return count;
}

char str_batt_status[5][40] = {
...
int asus_battery_info_proc_read(char *page, char **start, off_t off,
...
int asus_battery_profile_read(char *page, char **start, off_t off,
                    int count, int *eof, void *date)
{
    int ret;
...
int asus_battery_register_proc_fs_test(void)
{
    ...
    if (tmp_batt_info.test_flag & TEST_INFO_PROC_DUMP) {
        ...
        entry = create_proc_entry(PROCFS_BATTERY, 0640, NULL);
        if (!entry) {
