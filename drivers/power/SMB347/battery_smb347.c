...
/*
 * if using set USB to HC mode and set USB5/1/HC to register control
 */
#define USB5_1_HC_MAX                1
...
#define CFG_434V				BIT(1)|BIT(3)|BIT(5)
...
int smb347_dump_registers(struct seq_file *s);
bool smb347_has_charger_error(void);
int asus_charger_is_charging(void);
int smb347_soc_control_jeita(void);
void aicl_dete_worker(struct work_struct *dat);
static int smb347_read(struct smb347_charger *smb, u8 reg);
static int smb347_soc_detect_batt_tempr(int usb_state);
static inline int get_battery_rsoc(int *rsoc);

DEFINE_MUTEX(g_usb_state_lock);
int g_usb_state = CABLE_OUT;
EXPORT_SYMBOL(g_usb_state);
...
static void aicl_cur_control(int usb_state) {
	...
	if (aicl_result > 500) {
#if 0
        dev_info(&smb347_dev->client->dev, "%s: do nothing when aicl result(%dmA) > 500mA.\n", __func__, aicl_result);
#endif
		return;
	} else {
		setSMB347Charger(usb_state);
		dev_info(&smb347_dev->client->dev, "%s: execute AICL routine control work aicl result =%d\n", aicl_result, __func__);
	}
}
...
void aicl_dete_worker(struct work_struct *dat) {
	...
	smb347_soc_detect_batt_tempr(usb_state);

	/* acquire battery rsoc here */
	if (get_battery_rsoc(&rsoc)) {
		dev_err(&smb347_dev->client->dev, " %s: fail to get battery rsoc\n", __func__);
	} else {
		if (rsoc==1)
			smb347_dump_registers(NULL);
	}

	if (dat)
		queue_delayed_work(smb347_dev->chrgr_work_queue, &smb347_dev->aicl_dete_work, 30*HZ);
}
...
static int smb347_masked_write(struct i2c_client *client, int reg, u8 mask, u8 val) {
	s32 rc;
	u8 temp;
...
static int otg(int toggle) {
	...
	if (toggle) {
		...
		/* 3.OTG function change to PIN control */
                ret = smb347_update_write( smb347_dev,
                                           OTHER_CTRL_A_REG,
                                           BIT(6)|BIT(7),
                                           BIT(6));
		if (ret) {
			...
			return ret;
                ...
                /* 4.Enable OTG Function*/
                gpio_set_value(CHG_OTG_GPIO, 1);

		if (!gpio_get_value(CHG_OTG_GPIO)) {
			pr_err("Failed to set CHG_OTG pin ret=%d\n", ret);
			return ret;
		}

                /* 5.Set OTG current limit to 500mA & UVLO 2.7V*/
		// Refer to SMB347 Application Note 72 to solve serious problems
                ret = smb347_update_write( smb347_dev,
                                           OTG_TLIM_THERM_CNTRL_REG,
                                           BIT(0)|BIT(1)|BIT(2)|BIT(3),
		...
		printk(KERN_INFO "%s OTG DISABLE !!!\n", __func__);
		...
		// Refer to SMB347 Application Note 72 to solve serious problems
                ret = smb347_update_write( smb347_dev,
                                           ...
                                           BIT(0)|BIT(1)|BIT(2)|BIT(3),
                                           BIT(2));
		/* 2.Disable OTG Function*/
                gpio_set_value(CHG_OTG_GPIO, 0);
...
extern void ug31xx_cable_callback(unsigned usb_cable_state);
...
static struct power_supply smb347_power_supplies[] = {
	{
		.name = "ac",
		...
		.get_property = smb347_power_get_property,
	},
	{
		.name = "usb",
...
static int smb347_power_get_property(struct power_supply *psy,
	...
	case POWER_SUPPLY_PROP_PRESENT:
		if (psy->type == POWER_SUPPLY_TYPE_USB) {
			...
			chrg_status = asus_charger_is_charging();
...
int request_power_supply_changed() {
	...
	if (!smb347_dev) {
		pr_err("Warning: smb347_dev is null due to probe function has error\n");
		return -1;
	}

	power_supply_changed(&smb347_power_supplies[CHARGER_AC-1]);
	power_supply_changed(&smb347_power_supplies[CHARGER_USB-1]);

	return ret;
}

static int smb347_register_power_supply(struct device *dev) {
	int ret;

	ret = power_supply_register(dev, &smb347_power_supplies[CHARGER_USB-1]);
	if (ret) {
		BAT_DBG_E("Fail to register power supply USB\n");
		goto batt_err_reg_fail_usb;
	}

	ret = power_supply_register(dev, &smb347_power_supplies[CHARGER_AC-1]);
	if (ret) {
...
batt_err_reg_fail_ac:
	power_supply_unregister(&smb347_power_supplies[CHARGER_USB-1]);
...
static void do_charger(struct work_struct *work) {
	...
	msleep(10);
    /* 1.set fast charge current =1.8A set pre-charge current =150mA
       2.set cold soft limit current = 700mA
       3.Set Battery OV does not end charging cycle.
       4.Set Float Voltage = 4.34V
   */
	if (batt_info.cable_status == USB_ADAPTER || batt_info.cable_status == USB_PC) {
		if (smb347_dev && !gpio_get_value(smb347_dev->pdata->inok_gpio)) {
			...
			smb347_set_voltage(4340);
			if (batt_info.cable_status == USB_ADAPTER) {
				pr_info("set I_USB_IN=1200mA\n");
				/*set I_USB_IN=1200mA */
				smb347_AC_in_current(1200, USB5_1_HC_MAX);
			}
			/*SOC control JEITA*/
			smb347_soc_control_jeita();
		}
	}
...
int smb347_soc_control_jeita(void) {
	...
	ret = smb347_masked_write(smb347_dev->client,
	...
	/* 2.Cancel soft-hot limit, write 07h[1:0]="00" */
 ...
 */
int smb347_charger_control_jeita(void) {
	int ret;
...
int smb347_set_charge_current(void) {
        int ret = 0;
	...
	/* Set Fast Charge current = 1800mA (00h[7:5]="100"), Config CFG_CHARGE_CURRENT register */
	/* Set Pre-Charge current = 150mA (00h[4:3]="01"), Config CFG_CHARGE_CURRENT register */
	/* Set Termination current = 250mA (00h[2:0]="101"), Config CFG_CHARGE_CURRENT register */
        ret = smb347_update_write( smb347_dev, CFG_CHARGE_CURRENT, 0xFF, BIT(7)|BIT(3)|BIT(2)|BIT(0));
	if (ret < 0)
		goto out;

        /* Set cold soft limit current = 700mA (0Ah[7:6]="01"),Config CFG_SOFT_LIMIT register */
        ret = smb347_update_write( smb347_dev, CFG_SOFT_LIMIT, BIT(6)|BIT(7), BIT(6));
	if (ret < 0)
		goto out;
...
static int smb347_set_USB5_1_HC_max_current(int c) {
    ...
    /* 2. Set I_USB_IN =1.2A (01h[3:0]="0100") */
	ret = smb347_read(smb347_dev, INPUT_CURRENT_LIMIT_REG);
	if (ret < 0)
		goto out;
...
int smb347_charging_toggle(charging_toggle_level_t level, bool on) {
	int ret = 0;
	int charging_toggle;
...
int asus_charge_enable(bool on) {
	int ret = 0;
...
out:
	mutex_unlock(&smb347_dev->lock);
	return ret;
}EXPORT_SYMBOL(asus_charge_enable);
...
static int smb347_otg_gpio_init(struct smb347_charger *smb) {
    ...
    ret = gpio_request_one(CHG_OTG_GPIO,
            GPIOF_OUT_INIT_LOW,
            "smb347_otg");
...
EXPORT_SYMBOL(asus_charger_is_charging);
...
static int smb347_get_chrg_voltage() {
    int ret, voltage;
    ...
    ret = smb347_read(smb347_dev, FLOAT_VOLTAGE_REG);
    if (ret < 0)
        return ret;
...
int do_battery_jeita(int batt_tempr, int batt_volt,int temp_zone_last) {
	...
	int ret, need_add_0 = 0, need_add_0_10 = 0, need_add_45_55 = 0, need_add_55 = 0;// temperature delay protection
	int chrg_voltage;
	...
	} else if (batt_tempr >= (100 + need_add_0_10) && batt_tempr < (450 - need_add_45_55)) {
		// 10 < Temp < 45
	...
	switch (battery_temp_zone) {
		case TEMP_0:
			...
			/* Charging Disable */
			ret = smb347_charging_toggle(JEITA, false);
			break;
		case TEMP_0_10:
			/* Vchg=4.34v , write 03h[5:0]="101010"*/
			ret = smb347_masked_write(smb347_dev->client,
			...
			/* Charging Enable */
			ret = smb347_charging_toggle(JEITA, true);
			break;
		case TEMP_45_55:
			chrg_voltage = smb347_get_chrg_voltage();
			if (chrg_voltage == 4340 && batt_volt >= 4100) {
				...
				/* set fast charge current = 1800mA, write 00h[7:5]="100"*/
				ret = smb347_masked_write(smb347_dev->client,
						0x00,
						BIT(5)|BIT(6)|BIT(7),
						BIT(7));
...
int smb347_battery_jeita(int batt_tempr, int batt_volt) {
	int ret = 0;
...
static int smb347_soc_detect_batt_tempr(int usb_state) {
	...
	ret = smb347_battery_jeita(batt_tempr, batt_volt);
...
static int smb347_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	smb347_pdata.inok_gpio = INOK_GPIO;
	dev->platform_data = &smb347_pdata;
	...
	ret = smb347_otg_gpio_init(smb);
	if (ret < 0)
		return ret;

	smb->running = true;
	smb->dentry = debugfs_create_file("smb347-regs", S_IRUSR, NULL, smb,  &smb347_debugfs_fops);
	smb347_dev = smb;
	...
	ret = smb347_register_power_supply(&client->dev);
	if (ret < 0)
		return ret;
