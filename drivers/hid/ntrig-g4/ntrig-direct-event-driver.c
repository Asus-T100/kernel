...
static int my_open(struct inode *inode, struct file *filep)
{
	...
	memory_unlock(&flag);
	if (!accept) {
		ntrig_err("ntrig-direct-event-driver inside %s: concurrent sessions unsupported - rejecting\n",
				__func__);
		return -EBUSY;
	}
	ntrig_dbg("ntrig-direct-event-driver inside %s, file mode=%d, num_open_files=%d\n",
	...
	loff_t *offp)
{
	...
	if (count < sizeof(struct mr_message_types_s))	{
		ntrig_err("ntrig-direct-event-driver inside %s: output buffer too small\n",
				__func__);
		return -EINVAL;
	}
	...
	memory_unlock(&flag);
	if (interrupted) {
		ntrig_dbg("ntrig-direct-event-driver inside %s: read interrupted by release callback (file was closed)\n",
			__func__);
		return -EINTR;
	}
	if (ready) {
		if (copy_to_user(buff, &data, sizeof(data)) != 0) {
			ntrig_err("ntrig-direct-event-driver: kernel -> userspace copy failed!\n");
			ret_count = -EFAULT;
		} else
...
int g4_bus_init_direct_events(void)
{
	int res;

	ntrig_dbg("ntrig-direct-event-driver inside %s\n", __func__);

	if (g4_setup_direct_events(on_message)) {
		ntrig_err("ntrig-direct-event-driver inside %s cannot start dispatcher\n",
				__func__);
		g4_setup_direct_events(NULL);
		return DTRG_FAILED;
	}
...
EXPORT_SYMBOL_GPL(g4_bus_init_direct_events);

void g4_bus_exit_direct_events(void)
{
	ntrig_dbg("ntrig-direct-event-driver inside %s\n", __func__);
...
EXPORT_SYMBOL_GPL(g4_bus_exit_direct_events);
