...
struct _ntrig_low_mt_finger_g3 {
...
/** describes a	single finger in a multi-touch report */
struct _ntrig_low_mt_finger_g4 {
	u8 flags; /* tip switch, in range, touch valid */
	u16 fingerIndex;
	u16 x;
#ifdef NTRIG_RAVEN_DIGITIZER
	u16 xoffset1;
#endif
	u16 y;
#ifdef NTRIG_RAVEN_DIGITIZER
...
struct _ntrig_mt_report_fingers_g3 {
	struct _ntrig_low_mt_finger_g3 fingers[MAX_MT_FINGERS_G3];
...
struct _ntrig_mt_report_fingers_g4 {
	struct _ntrig_low_mt_finger_g4 fingers[MAX_MT_FINGERS_G4];
	u8 contactCount; /* number of fingers */
} __packed;
...
struct _ntrig_low_pen_report {
	u8 reportId;
	/* bit 0: in range 1: tip switch 2: barrel (right click) 3: invert
	 * 4: eraser 5-7: unused */
	u8 flags;
	u16 x;
	u16 y;
 ...
 */
void g4_build_ncp_dfu_cmd(u8 cmd,
		const char *buf, short msg_len, char *out_buf);
