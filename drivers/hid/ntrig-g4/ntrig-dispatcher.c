	...
	struct _ntrig_dev_ncp_func *ncp_func)
{
	...
	for (i = 0; i < MAX_NUMBER_OF_SENSORS; i++) {
		...
		if (is_same_bus_id(ntrig_devices_hid[i].bus_id, bus_id) ||
			...
			case TYPE_BUS_SPI_HID:
			case TYPE_BUS_I2C_HID:
				g4_set_ntrig_devices_hid_data(i, dev_type,
						bus_id, ncp_func);
				sensor_id = i;
				break;
			case TYPE_BUS_USB_RAW:
			case TYPE_BUS_SPI_RAW:
			case TYPE_BUS_I2C_RAW:
				g4_set_ntrig_devices_raw_data(i, dev_type,
						bus_id, ncp_func);
				sensor_id = i;
				break;
			default:
				ntrig_err("inside %s - FAILED TO REGISTER - illegal dev type: %d\n",
						__func__, dev_type);
				ret = DTRG_FAILED;
			}
	...
	if (i >= MAX_NUMBER_OF_SENSORS) {
		ntrig_err("inside %s - FAILED TO REGISTER -  device for all sensors already exists !!\n",
				__func__);
		return DTRG_FAILED;
	}
...
void g4_release_ntrig_devices_io(__u8 sensor_id, int dev_type, char *bus_id)
{
	...
	case TYPE_BUS_I2C_RAW:
		...
		break;
	default:
		ntrig_err("inside %s - FAILED TO UNREGISTER -  illegal dev type: %d\n",
				__func__, dev_type);
		break;
	}
...
void g4_UnregNtrigDispatcher(void *dev, __u8 sensor_id,
...
int g4_create_single_touch(struct _ntrig_bus_device *dev, __u8 sensor_id)
{
	...
	if (!input_device) {
		ntrig_err("ntrig inside %s ERROR! : cdev_alloc() error!!! no memory!!\n",
				__func__);
		return DTRG_FAILED;
	}
	...
	input_set_abs_params(input_device, ABS_X,
		...
		dev->logical_max_x - g4_get_touch_screen_border_pen_right(),
		0, 0);
...
static int get_finger_index(int id, int allocate_if_new)
{
	...
	if (allocate_if_new) {
		...
		ntrig_err("%s: cannot find index for finger, all slots in use\n",
				__func__);
	}
...
static void check_for_missed_remove_fingers(struct input_dev *input)
{
	...
	if (missed) {
		ntrig_err("%s: fingers in previous report not exist in current report, missed finger mask is %d\n",
				__func__, missed);
		for (j = 0; missed; j++) {
			if (missed & 1) {
	...
	struct mr_message_types_s *multi_touch)
{
	...
	for (i = 0; i < fingers_num; i++) {
		...
		if (fingers[i].removed) {
			ntrig_dbg("finger removed: number %d, track_id %d, slot %d\n",
					i, fingers[i].track_id, slot);
			unMarkBit(&gPreviousFingers, slot);
			remove_slot_typeB(input, slot);
		} else {
			ntrig_dbg("finger NOT removed: number %d, track_id %d, slot %d\n",
					i, fingers[i].track_id, slot);
...
{
	pr_info("Dispatcher Driver Version %s\n", NTRIG_DRIVER_VERSION);
