	...
	struct input_dev *input_device)
{
	...
	if (g4_virtual_keys_supported()) {
		ntrig_dbg("inside %s with VIRTUAL_KEYS_SUPPORTED1\n", __func__);

		__set_bit(EV_KEY, input_device->evbit);
		for (i = 0; i < g4_get_virtual_keys_num(); i++)
			__set_bit(g4_get_virt_keys_scan_code(i),
				input_device->keybit);

		input_set_abs_params(input_device, ABS_MT_POSITION_X,
			dev->logical_min_x+g4_get_touch_screen_border_left(),
			dev->logical_max_x-g4_get_touch_screen_border_right(),
			0, 0);
		input_set_abs_params(input_device, ABS_MT_POSITION_Y,
			dev->logical_min_y+g4_get_touch_screen_border_down(),
			dev->logical_max_y-g4_get_touch_screen_border_up(),
			0, 0);
